...
`),commands:e,projectDir:s};return console.log("✅ Terminal commands executed successfully"),o}catch(o){return console.error("❌ Terminal command execution failed:",o),{success:!1,output:`Error: ${o.message}`,commands:e,error:o.message}}}}const Zt=new Zd,eu=()=>{const{currentProject:t,switchFile:e,updateFile:s,saveProject:n,createNewProject:o,updateConfig:i}=lt(),[a,l]=p.useState(!1),[c,d]=p.useState(""),[m,u]=p.useState(!1),[g,y]=p.useState(!1),[f,w]=p.useState(!1),[h,C]=p.useState("firebase"),[j,v]=p.useState(!1),[b,x]=p.useState(""),[T,A]=p.useState({show:!1,x:0,y:0,filename:""}),S=p.useRef(null),M={"index.html":"🌐","style.css":"🎨","script.js":"⚡","components.jsx":"🧩","package.json":"📦","README.md":"📖","server.js":"🖥️","database.js":"🗄️","auth.js":"🔐","api.js":"🔌"},E=k=>M[k]||"📄",P=k=>{e(k)},W=(k,L)=>{k.preventDefault(),A({show:!0,x:k.clientX,y:k.clientY,filename:L})},B=k=>{const{filename:L}=T;switch(A({show:!1,x:0,y:0,filename:""}),k){case"download":J(L);break;case"delete":_(L);break;case"rename":U.info("Rename functionality coming soon!");break;case"copy":U.info("Copy functionality coming soon!");break}},D=()=>{A({show:!1,x:0,y:0,filename:""})};p.useEffect(()=>{const k=L=>{S.current&&!S.current.contains(L.target)&&D()};return T.show&&document.addEventListener("mousedown",k),()=>{document.removeEventListener("mousedown",k)}},[T.show]);const F=()=>{if(c.trim()){const k=c.includes(".")?c:`${c}.js`;s(k,""),e(k),d(""),l(!1),U.success(`Created ${k}`)}},_=k=>{if(Object.keys(t.files).length<=1){U.error("Cannot delete the last file");return}if(confirm(`Delete ${k}?`)){const L={...t.files};if(delete L[k],Object.keys(L).forEach(O=>{t.files[O]=L[O]}),t.activeFile===k){const O=Object.keys(L);e(O[0])}U.success(`Deleted ${k}`)}},J=k=>{const L=t.files[k]||"",O=new Blob([L],{type:"text/plain"}),te=URL.createObjectURL(O),Pe=document.createElement("a");Pe.href=te,Pe.download=k,document.body.appendChild(Pe),Pe.click(),document.body.removeChild(Pe),URL.revokeObjectURL(te),U.success(`Downloaded ${k}`)},q=()=>{const k={name:t.name,files:t.files,config:t.config,timestamp:new Date().toISOString()},L=new Blob([JSON.stringify(k,null,2)],{type:"application/json"}),O=URL.createObjectURL(L),te=document.createElement("a");te.href=O,te.download=`${t.name}.json`,document.body.appendChild(te),te.click(),document.body.removeChild(te),URL.revokeObjectURL(O),U.success("Project downloaded!")},le=k=>{const L=k.target.files[0];if(L){const O=new FileReader;O.onload=te=>{s(L.name,te.target.result),e(L.name),U.success(`Uploaded ${L.name}`)},O.readAsText(L)}},xe=async()=>{if(!b.trim()){U.error("Please enter a project name");return}if(Object.keys(t.files).length===0){U.error("Please generate some code first");return}w(!0);try{const k=await $(t,b),L=[];if(j){U.info("Deploying to both Firebase and GitHub...");const[O,te]=await Promise.allSettled([Zt.deployToFirebase(k,b),Zt.deployToGitHub(k,b)]);if(O.status==="fulfilled"&&O.value.success&&L.push({platform:"Firebase",...O.value}),te.status==="fulfilled"&&te.value.success&&L.push({platform:"GitHub",...te.value}),L.length===2)U.success("Successfully deployed to both Firebase and GitHub!");else if(L.length===1)U.success(`Deployed to ${L[0].platform} (${L.length===1?"one":"both"} platform${L.length===1?"":"s"} failed)`);else throw new Error("Both deployments failed")}else{let O;h==="firebase"?O=await Zt.deployToFirebase(k,b):h==="github"&&(O=await Zt.deployToGitHub(k,b)),O.success&&(L.push({platform:h,...O}),U.success(`Successfully deployed to ${O.platform}!`))}L.forEach(O=>{O.url&&window.open(O.url,"_blank"),O.instructions&&(console.log(`${O.platform} deployment instructions:`,O.instructions),U.success(`Check console for ${O.platform} Pages setup instructions`))}),y(!1),x(""),v(!1)}catch(k){U.error(`Deployment failed: ${k.message}`)}finally{w(!1)}},$=async(k,L)=>{const O={...k};return O.files["index.html"]||(O.files["index.html"]=G(L)),O.files["package.json"]||(O.files["package.json"]=V(L,O.config)),O.files["README.md"]||(O.files["README.md"]=ne(L,O.config)),O.files["index.html"]=I(O.files["index.html"],L),O.files["manifest.json"]||(O.files["manifest.json"]=ie(L)),O},G=k=>`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`,V=(k,L)=>JSON.stringify({name:k.toLowerCase().replace(/[^a-z0-9-]/g,"-"),version:"1.0.0",description:`Built with DreamBuild - ${k}`,main:"index.html",scripts:{start:"npx serve .",build:"echo 'Static site - no build required'",deploy:"echo 'Deploy using DreamBuild deployment system'"},keywords:["dreambuild","ai-generated","web-app",L.appType||"frontend"],author:"DreamBuild AI",license:"MIT",engines:{node:">=14.0.0"},dependencies:{},devDependencies:{serve:"^14.0.0"}},null,2),ne=(k,L)=>{var O;return`# ${k}
...
${P}`),console.log("🎮 Preview Debug - Final HTML length:",P.length),console.log("🎮 Preview Debug - HTML contains <style>:",P.includes("<style>")),console.log("🎮 Preview Debug - HTML contains <script>:",P.includes("<script>")),console.log("🎮 Preview Debug - HTML preview:",P.substring(0,500)+"...");const W=e.current;W.srcdoc=P,W.onload=()=>{n(!1),l(null)},W.onerror=()=>{n(!1),l("Failed to load preview")}}catch(j){console.error("Preview update error:",j),n(!1),l("Preview update failed")}},u=j=>j?j.replace(/`/g,"\\`").replace(/\${/g,"\\${").replace(/\$/g,"\\$"):"",g=()=>{if(console.log("🎮 Creating React preview..."),!e.current){console.log("🎮 createReactPreview: iframeRef.current is null");return}try{const j=t.files["src/components/GameApp.jsx"]||"",v=t.files["src/components/GameComponent.jsx"]||"",b=t.files["src/components/TempleRunUI.jsx"]||"",x=t.files["src/components/RunnerPlayer.jsx"]||"",T=t.files["src/components/Obstacle.jsx"]||"",A=t.files["src/components/Coin.jsx"]||"",S=t.files["src/components/Player.jsx"]||"",M=t.files["src/components/GameApp.css"]||"",E=t.files["src/components/GameComponent.css"]||"",P=t.files["src/components/TempleRunUI.css"]||"",W=t.files["src/components/RunnerPlayer.css"]||"",B=t.files["src/components/Obstacle.css"]||"",D=t.files["src/components/Coin.css"]||"",F=t.files["src/components/Player.css"]||"",_=b||x||T,J=S||A,q=v.toLowerCase(),le=q.includes("temple run")||q.includes("lane")||q.includes("jump")||q.includes("slide")||q.includes("obstacle")||q.includes("endless runner"),xe=_||le,$=J&&!xe;console.log("🎮 Preview Debug - Available files:"),console.log("🎮 - templeRunUIFile:",!!b),console.log("🎮 - runnerPlayerFile:",!!x),console.log("🎮 - obstacleFile:",!!T),console.log("🎮 - playerFile:",!!S),console.log("🎮 - coinFile:",!!A),console.log("🎮 - hasTempleRunFiles:",_),console.log("🎮 - hasTempleRunContent:",le),console.log("🎮 - isTempleRun:",xe),console.log("🎮 - isCoinCollector:",$),console.log("🎮 - All project files:",Object.keys(t.files)),console.log("🎮 FINAL GAME TYPE DECISION:"),console.log(xe?"🎮 ✅ RENDERING TEMPLE RUN GAME":$?"🎮 ✅ RENDERING COIN COLLECTOR GAME":"🎮 ✅ RENDERING DEFAULT GAME");const G=`
    ...
    `,V=e.current;console.log("🎮 Setting iframe content, length:",G.length),V.srcdoc=G,V.onload=()=>{var ne,ie;console.log("🎮 Iframe loaded successfully"),n(!1),l(null);try{const I=((ie=(ne=V.contentDocument)==null?void 0:ne.body)==null?void 0:ie.textContent)||"";console.log("🎮 Iframe content verification - length:",I.length),console.log("🎮 Iframe content preview:",I.substring(0,100))}catch(I){console.log("🎮 Iframe content verification - access denied:",I.message)}},V.onerror=()=>{console.log("🎮 Iframe error occurred"),n(!1),l("Failed to load React preview")}}catch(j){console.error("🎮 Error in createReactPreview:",j),n(!1),l(`Preview generation failed: ${j.message}`)}},y=()=>{if(!e.current)return;const j=`
      <!DOCTYPE html>
      <html lang="en">
      <head>
        <meta charset="UTF-8">
...
${_}`},v=(D,F)=>{const _=F.getDropResult();if(!_)return;const J={id:`component-${Date.now()}`,type:D.type,name:D.name,position:{x:_.x,y:_.y},size:{width:200,height:100},styles:{position:"absolute",left:`${_.x}px`,top:`${_.y}px`,width:"200px",height:"100px",border:"1px solid #ddd",padding:"10px",backgroundColor:"#fff"},content:D.name,properties:{}};o(q=>[...q,J])},b=D=>{a(D),c(!0)},x=(D,F)=>{if(!i)return;const _={...i,[D]:F,styles:{...i.styles,[D]:F}};o(J=>J.map(q=>q.id===i.id?_:q)),a(_)},T=()=>`
.app {
  position: relative;
  width: 100%;
  min-height: 100vh;
...
`,A=()=>{const D={components:n,appCode:j(),cssCode:T(),metadata:{projectId:t,exportedAt:new Date().toISOString(),componentCount:n.length}},F=JSON.stringify(D,null,2),_=new Blob([F],{type:"application/json"}),J=URL.createObjectURL(_),q=document.createElement("a");q.href=J,q.download=`dreambuild-project-${t}.json`,q.click(),URL.revokeObjectURL(J)};return p.useEffect(()=>{e&&e({appCode:j(),cssCode:T(),components:n})},[n,e]),r.jsx(nf,{backend:Xf,children:r.jsxs("div",{className:"visual-editor",children:[r.jsxs("div",{className:"editor-header",children:[r.jsx("h2",{children:"🎨 Visual Editor"}),r.jsxs("div",{className:"editor-controls",children:[r.jsx("button",{className:"btn btn-secondary",onClick:()=>m(!d),children:d?"Edit":"Preview"}),r.jsx("button",{className:"btn btn-primary",onClick:A,children:"Export"})]})]}),r.jsxs("div",{className:"editor-layout",children:[r.jsxs("div",{className:"component-library",children:[r.jsx("h3",{children:"📚 Component Library"}),r.jsx("div",{className:"library-categories",children:["Layout","Content","Interactive","Form","Media","Data","Navigation","Overlay"].map(D=>r.jsxs("div",{className:"category",children:[r.jsx("h4",{children:D}),r.jsx("div",{className:"category-components",children:h.filter(F=>F.category===D).map(F=>r.jsx(Zf,{type:F.type,name:F.name,icon:F.icon},F.type))})]},D))})]}),r.jsxs("div",{className:"canvas-container",children:[r.jsxs("div",{className:"canvas-toolbar",children:[r.jsxs("div",{className:"canvas-controls",children:[r.jsx("button",{className:"btn btn-sm",onClick:()=>f(y*.8),children:"Zoom Out"}),r.jsxs("span",{className:"zoom-level",children:[Math.round(y*100),"%"]}),r.jsx("button",{className:"btn btn-sm",onClick:()=>f(y*1.2),children:"Zoom In"})]}),r.jsx("div",{className:"canvas-size",children:r.jsxs("select",{value:`${u.width}x${u.height}`,onChange:D=>{const[F,_]=D.target.value.split("x").map(Number);g({width:F,height:_})},children:[r.jsx("option",{value:"1200x800",children:"Desktop (1200x800)"}),r.jsx("option",{value:"768x1024",children:"Tablet (768x1024)"}),r.jsx("option",{value:"375x667",children:"Mobile (375x667)"})]})})]}),r.jsx("div",{className:"canvas",ref:w,style:{width:u.width*y,height:u.height*y,transform:`scale(${y})`,transformOrigin:"top left"},children:r.jsx(eg,{onDrop:v,children:n.map(D=>r.jsx(tg,{component:D,onSelect:b,isSelected:(i==null?void 0:i.id)===D.id},D.id))})})]}),l&&i&&r.jsxs("div",{className:"properties-panel",children:[r.jsx("h3",{children:"⚙️ Properties"}),r.jsxs("div",{className:"property-groups",children:[r.jsxs("div",{className:"property-group",children:[r.jsx("h4",{children:"Content"}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"Text Content"}),r.jsx("input",{type:"text",value:i.content||"",onChange:D=>x("content",D.target.value)})]})]}),r.jsxs("div",{className:"property-group",children:[r.jsx("h4",{children:"Position"}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"X Position"}),r.jsx("input",{type:"number",value:((S=i.position)==null?void 0:S.x)||0,onChange:D=>x("left",`${D.target.value}px`)})]}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"Y Position"}),r.jsx("input",{type:"number",value:((M=i.position)==null?void 0:M.y)||0,onChange:D=>x("top",`${D.target.value}px`)})]})]}),r.jsxs("div",{className:"property-group",children:[r.jsx("h4",{children:"Size"}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"Width"}),r.jsx("input",{type:"number",value:((E=i.size)==null?void 0:E.width)||200,onChange:D=>x("width",`${D.target.value}px`)})]}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"Height"}),r.jsx("input",{type:"number",value:((P=i.size)==null?void 0:P.height)||100,onChange:D=>x("height",`${D.target.value}px`)})]})]}),r.jsxs("div",{className:"property-group",children:[r.jsx("h4",{children:"Style"}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"Background Color"}),r.jsx("input",{type:"color",value:((W=i.styles)==null?void 0:W.backgroundColor)||"#ffffff",onChange:D=>x("backgroundColor",D.target.value)})]}),r.jsxs("div",{className:"property",children:[r.jsx("label",{children:"Border Color"}),r.jsx("input",{type:"color",value:((B=i.styles)==null?void 0:B.borderColor)||"#dddddd",onChange:D=>x("borderColor",D.target.value)})]})]})]})]})]}),r.jsx("style",{jsx:!0,children:`
          .visual-editor {
            height: 100vh;
            display: flex;
            flex-direction: column;
