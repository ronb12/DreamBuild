...
`),commands:e,projectDir:s};return console.log("✅ Terminal commands executed successfully"),o}catch(o){return console.error("❌ Terminal command execution failed:",o),{success:!1,output:`Error: ${o.message}`,commands:e,error:o.message}}}}const rr=new su,nu=()=>{const{currentProject:r,switchFile:e,updateFile:s,saveProject:n,createNewProject:o,updateConfig:i}=Ye(),[a,l]=p.useState(!1),[c,d]=p.useState(""),[m,u]=p.useState(!1),[g,b]=p.useState(!1),[f,w]=p.useState(!1),[h,k]=p.useState("firebase"),[N,v]=p.useState(!1),[y,x]=p.useState(""),[T,A]=p.useState({show:!1,x:0,y:0,filename:""}),C=p.useRef(null),L={"index.html":"🌐","style.css":"🎨","script.js":"⚡","components.jsx":"🧩","package.json":"📦","README.md":"📖","server.js":"🖥️","database.js":"🗄️","auth.js":"🔐","api.js":"🔌"},E=j=>L[j]||"📄",P=j=>{e(j)},Y=(j,M)=>{j.preventDefault(),A({show:!0,x:j.clientX,y:j.clientY,filename:M})},z=j=>{const{filename:M}=T;switch(A({show:!1,x:0,y:0,filename:""}),j){case"download":q(M);break;case"delete":F(M);break;case"rename":$.info("Rename functionality coming soon!");break;case"copy":$.info("Copy functionality coming soon!");break}},D=()=>{A({show:!1,x:0,y:0,filename:""})};p.useEffect(()=>{const j=M=>{C.current&&!C.current.contains(M.target)&&D()};return T.show&&document.addEventListener("mousedown",j),()=>{document.removeEventListener("mousedown",j)}},[T.show]);const I=()=>{if(c.trim()){const j=c.includes(".")?c:`${c}.js`;s(j,""),e(j),d(""),l(!1),$.success(`Created ${j}`)}},F=j=>{if(Object.keys(r.files).length<=1){$.error("Cannot delete the last file");return}if(confirm(`Delete ${j}?`)){const M={...r.files};if(delete M[j],Object.keys(M).forEach(R=>{r.files[R]=M[R]}),r.activeFile===j){const R=Object.keys(M);e(R[0])}$.success(`Deleted ${j}`)}},q=j=>{const M=r.files[j]||"",R=new Blob([M],{type:"text/plain"}),V=URL.createObjectURL(R),re=document.createElement("a");re.href=V,re.download=j,document.body.appendChild(re),re.click(),document.body.removeChild(re),URL.revokeObjectURL(V),$.success(`Downloaded ${j}`)},G=()=>{const j={name:r.name,files:r.files,config:r.config,timestamp:new Date().toISOString()},M=new Blob([JSON.stringify(j,null,2)],{type:"application/json"}),R=URL.createObjectURL(M),V=document.createElement("a");V.href=R,V.download=`${r.name}.json`,document.body.appendChild(V),V.click(),document.body.removeChild(V),URL.revokeObjectURL(R),$.success("Project downloaded!")},le=j=>{const M=j.target.files[0];if(M){const R=new FileReader;R.onload=V=>{s(M.name,V.target.result),e(M.name),$.success(`Uploaded ${M.name}`)},R.readAsText(M)}},he=async()=>{if(!y.trim()){$.error("Please enter a project name");return}if(Object.keys(r.files).length===0){$.error("Please generate some code first");return}w(!0);try{const j=await _(r,y),M=[];if(N){$.info("Deploying to both Firebase and GitHub...");const[R,V]=await Promise.allSettled([rr.deployToFirebase(j,y),rr.deployToGitHub(j,y)]);if(R.status==="fulfilled"&&R.value.success&&M.push({platform:"Firebase",...R.value}),V.status==="fulfilled"&&V.value.success&&M.push({platform:"GitHub",...V.value}),M.length===2)$.success("Successfully deployed to both Firebase and GitHub!");else if(M.length===1)$.success(`Deployed to ${M[0].platform} (${M.length===1?"one":"both"} platform${M.length===1?"":"s"} failed)`);else throw new Error("Both deployments failed")}else{let R;h==="firebase"?R=await rr.deployToFirebase(j,y):h==="github"&&(R=await rr.deployToGitHub(j,y)),R.success&&(M.push({platform:h,...R}),$.success(`Successfully deployed to ${R.platform}!`))}M.forEach(R=>{R.url&&window.open(R.url,"_blank"),R.instructions&&(console.log(`${R.platform} deployment instructions:`,R.instructions),$.success(`Check console for ${R.platform} Pages setup instructions`))}),b(!1),x(""),v(!1)}catch(j){$.error(`Deployment failed: ${j.message}`)}finally{w(!1)}},_=async(j,M)=>{const R={...j};return R.files["index.html"]||(R.files["index.html"]=H(M)),R.files["package.json"]||(R.files["package.json"]=W(M,R.config)),R.files["README.md"]||(R.files["README.md"]=ne(M,R.config)),R.files["index.html"]=Oe(R.files["index.html"],M),R.files["manifest.json"]||(R.files["manifest.json"]=oe(M)),R},H=j=>`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`,W=(j,M)=>JSON.stringify({name:j.toLowerCase().replace(/[^a-z0-9-]/g,"-"),version:"1.0.0",description:`Built with DreamBuild - ${j}`,main:"index.html",scripts:{start:"npx serve .",build:"echo 'Static site - no build required'",deploy:"echo 'Deploy using DreamBuild deployment system'"},keywords:["dreambuild","ai-generated","web-app",M.appType||"frontend"],author:"DreamBuild AI",license:"MIT",engines:{node:">=14.0.0"},dependencies:{},devDependencies:{serve:"^14.0.0"}},null,2),ne=(j,M)=>{var R;return`# ${j}
