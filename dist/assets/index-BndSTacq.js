...
`),commands:e,projectDir:s};return console.log("✅ Terminal commands executed successfully"),o}catch(o){return console.error("❌ Terminal command execution failed:",o),{success:!1,output:`Error: ${o.message}`,commands:e,error:o.message}}}}const sr=new au,lu=()=>{const{currentProject:r,switchFile:e,updateFile:s,saveProject:n,createNewProject:o,updateConfig:i}=Le(),[a,l]=p.useState(!1),[d,c]=p.useState(""),[u,m]=p.useState(!1),[x,v]=p.useState(!1),[g,f]=p.useState(!1),[h,S]=p.useState("firebase"),[b,j]=p.useState(!1),[w,y]=p.useState(""),[T,A]=p.useState({show:!1,x:0,y:0,filename:""}),k=p.useRef(null),L={"index.html":"🌐","style.css":"🎨","script.js":"⚡","components.jsx":"🧩","package.json":"📦","README.md":"📖","server.js":"🖥️","database.js":"🗄️","auth.js":"🔐","api.js":"🔌"},E=N=>L[N]||"📄",P=N=>{e(N)},Y=(N,M)=>{N.preventDefault(),A({show:!0,x:N.clientX,y:N.clientY,filename:M})},H=N=>{const{filename:M}=T;switch(A({show:!1,x:0,y:0,filename:""}),N){case"download":q(M);break;case"delete":F(M);break;case"rename":_.info("Rename functionality coming soon!");break;case"copy":_.info("Copy functionality coming soon!");break}},D=()=>{A({show:!1,x:0,y:0,filename:""})};p.useEffect(()=>{const N=M=>{k.current&&!k.current.contains(M.target)&&D()};return T.show&&document.addEventListener("mousedown",N),()=>{document.removeEventListener("mousedown",N)}},[T.show]);const I=()=>{if(d.trim()){const N=d.includes(".")?d:`${d}.js`;s(N,""),e(N),c(""),l(!1),_.success(`Created ${N}`)}},F=N=>{if(Object.keys(r.files).length<=1){_.error("Cannot delete the last file");return}if(confirm(`Delete ${N}?`)){const M={...r.files};if(delete M[N],Object.keys(M).forEach(O=>{r.files[O]=M[O]}),r.activeFile===N){const O=Object.keys(M);e(O[0])}_.success(`Deleted ${N}`)}},q=N=>{const M=r.files[N]||"",O=new Blob([M],{type:"text/plain"}),J=URL.createObjectURL(O),re=document.createElement("a");re.href=J,re.download=N,document.body.appendChild(re),re.click(),document.body.removeChild(re),URL.revokeObjectURL(J),_.success(`Downloaded ${N}`)},G=()=>{const N={name:r.name,files:r.files,config:r.config,timestamp:new Date().toISOString()},M=new Blob([JSON.stringify(N,null,2)],{type:"application/json"}),O=URL.createObjectURL(M),J=document.createElement("a");J.href=O,J.download=`${r.name}.json`,document.body.appendChild(J),J.click(),document.body.removeChild(J),URL.revokeObjectURL(O),_.success("Project downloaded!")},le=N=>{const M=N.target.files[0];if(M){const O=new FileReader;O.onload=J=>{s(M.name,J.target.result),e(M.name),_.success(`Uploaded ${M.name}`)},O.readAsText(M)}},ge=async()=>{if(!w.trim()){_.error("Please enter a project name");return}if(Object.keys(r.files).length===0){_.error("Please generate some code first");return}f(!0);try{const N=await $(r,w),M=[];if(b){_.info("Deploying to both Firebase and GitHub...");const[O,J]=await Promise.allSettled([sr.deployToFirebase(N,w),sr.deployToGitHub(N,w)]);if(O.status==="fulfilled"&&O.value.success&&M.push({platform:"Firebase",...O.value}),J.status==="fulfilled"&&J.value.success&&M.push({platform:"GitHub",...J.value}),M.length===2)_.success("Successfully deployed to both Firebase and GitHub!");else if(M.length===1)_.success(`Deployed to ${M[0].platform} (${M.length===1?"one":"both"} platform${M.length===1?"":"s"} failed)`);else throw new Error("Both deployments failed")}else{let O;h==="firebase"?O=await sr.deployToFirebase(N,w):h==="github"&&(O=await sr.deployToGitHub(N,w)),O.success&&(M.push({platform:h,...O}),_.success(`Successfully deployed to ${O.platform}!`))}M.forEach(O=>{O.url&&window.open(O.url,"_blank"),O.instructions&&(console.log(`${O.platform} deployment instructions:`,O.instructions),_.success(`Check console for ${O.platform} Pages setup instructions`))}),v(!1),y(""),j(!1)}catch(N){_.error(`Deployment failed: ${N.message}`)}finally{f(!1)}},$=async(N,M)=>{const O={...N};return O.files["index.html"]||(O.files["index.html"]=z(M)),O.files["package.json"]||(O.files["package.json"]=V(M,O.config)),O.files["README.md"]||(O.files["README.md"]=ne(M,O.config)),O.files["index.html"]=Pe(O.files["index.html"],M),O.files["manifest.json"]||(O.files["manifest.json"]=oe(M)),O},z=N=>`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`,console.log("🎮 Preview Debug - Created basic HTML structure")}let P=b;if(w.trim()&&(P=P.replace(/<link[^>]*rel=["']stylesheet["'][^>]*href=["']style\.css["'][^>]*>/gi,`<style>${w}</style>`),P===b&&P.includes("<head>")?P=P.replace("<head>",`<head>
<style>${w}</style>`):P===b&&!P.includes("<head>")&&(P.includes("<title>")?P=P.replace("</title>",`</title>
<style>${w}</style>`):P=`<style>${w}</style>
${P}`)),y.trim()){const H=`(function() {
          ${y}
...
${P}`),console.log("🎮 Preview Debug - Final HTML length:",P.length),console.log("🎮 Preview Debug - HTML contains <style>:",P.includes("<style>")),console.log("🎮 Preview Debug - HTML contains <script>:",P.includes("<script>")),console.log("🎮 Preview Debug - HTML preview:",P.substring(0,500)+"...");const Y=e.current;Y.srcdoc=P,Y.onload=()=>{n(!1),l(null)},Y.onerror=()=>{n(!1),l("Failed to load preview")}}catch(b){console.error("Preview update error:",b),n(!1),l("Preview update failed")}},m=b=>b?b.replace(/`/g,"\\`").replace(/\${/g,"\\${").replace(/\$/g,"\\$"):"",x=()=>{if(console.log("🎮 Creating React preview..."),!e.current){console.log("🎮 createReactPreview: iframeRef.current is null");return}try{const b=r.files["src/components/GameApp.jsx"]||"",j=r.files["src/components/GameComponent.jsx"]||"",w=r.files["src/components/TempleRunUI.jsx"]||"",y=r.files["src/components/RunnerPlayer.jsx"]||"",T=r.files["src/components/Obstacle.jsx"]||"",A=r.files["src/components/Coin.jsx"]||"",k=r.files["src/components/Player.jsx"]||"",L=r.files["src/components/GameApp.css"]||"",E=r.files["src/components/GameComponent.css"]||"",P=r.files["src/components/TempleRunUI.css"]||"",Y=r.files["src/components/RunnerPlayer.css"]||"",H=r.files["src/components/Obstacle.css"]||"",D=r.files["src/components/Coin.css"]||"",I=r.files["src/components/Player.css"]||"",F=w||y||T,q=k||A,G=j.toLowerCase(),le=G.includes("temple run")||G.includes("lane")||G.includes("jump")||G.includes("slide")||G.includes("obstacle")||G.includes("endless runner"),ge=F||le,$=q&&!ge;console.log("🎮 Preview Debug - Available files:"),console.log("🎮 - templeRunUIFile:",!!w),console.log("🎮 - runnerPlayerFile:",!!y),console.log("🎮 - obstacleFile:",!!T),console.log("🎮 - playerFile:",!!k),console.log("🎮 - coinFile:",!!A),console.log("🎮 - hasTempleRunFiles:",F),console.log("🎮 - hasTempleRunContent:",le),console.log("🎮 - isTempleRun:",ge),console.log("🎮 - isCoinCollector:",$),console.log("🎮 - All project files:",Object.keys(r.files)),console.log("🎮 FINAL GAME TYPE DECISION:"),console.log(ge?"🎮 ✅ RENDERING TEMPLE RUN GAME":$?"🎮 ✅ RENDERING COIN COLLECTOR GAME":"🎮 ✅ RENDERING DEFAULT GAME");const z=`
      <!DOCTYPE html>
      <html lang="en">
      <head>
        <meta charset="UTF-8">
...
</div>`})[D.type]||`<div>Unknown component: ${D.type}</div>`,b=()=>{const D=`import React, { useState, useEffect } from 'react';
...
${F}`},j=(D,I)=>{const F=I.getDropResult();if(!F)return;const q={id:`component-${Date.now()}`,type:D.type,name:D.name,position:{x:F.x,y:F.y},size:{width:200,height:100},styles:{position:"absolute",left:`${F.x}px`,top:`${F.y}px`,width:"200px",height:"100px",border:"1px solid #ddd",padding:"10px",backgroundColor:"#fff"},content:D.name,properties:{}};o(G=>[...G,q])},w=D=>{a(D),d(!0)},y=(D,I)=>{if(!i)return;const F={...i,[D]:I,styles:{...i.styles,[D]:I}};o(q=>q.map(G=>G.id===i.id?F:G)),a(F)},T=()=>`
.app {
  position: relative;
  width: 100%;
  min-height: 100vh;
...
`,A=()=>{const D={components:n,appCode:b(),cssCode:T(),metadata:{projectId:r,exportedAt:new Date().toISOString(),componentCount:n.length}},I=JSON.stringify(D,null,2),F=new Blob([I],{type:"application/json"}),q=URL.createObjectURL(F),G=document.createElement("a");G.href=q,G.download=`dreambuild-project-${r}.json`,G.click(),URL.revokeObjectURL(q)};return p.useEffect(()=>{e&&e({appCode:b(),cssCode:T(),components:n})},[n,e]),t.jsx(pf,{backend:og,children:t.jsxs("div",{className:"visual-editor",children:[t.jsxs("div",{className:"editor-header",children:[t.jsx("h2",{children:"🎨 Visual Editor"}),t.jsxs("div",{className:"editor-controls",children:[t.jsx("button",{className:"btn btn-secondary",onClick:()=>u(!c),children:c?"Edit":"Preview"}),t.jsx("button",{className:"btn btn-primary",onClick:A,children:"Export"})]})]}),t.jsxs("div",{className:"editor-layout",children:[t.jsxs("div",{className:"component-library",children:[t.jsx("h3",{children:"📚 Component Library"}),t.jsx("div",{className:"library-categories",children:["Layout","Content","Interactive","Form","Media","Data","Navigation","Overlay"].map(D=>t.jsxs("div",{className:"category",children:[t.jsx("h4",{children:D}),t.jsx("div",{className:"category-components",children:h.filter(I=>I.category===D).map(I=>t.jsx(ag,{type:I.type,name:I.name,icon:I.icon},I.type))})]},D))})]}),t.jsxs("div",{className:"canvas-container",children:[t.jsxs("div",{className:"canvas-toolbar",children:[t.jsxs("div",{className:"canvas-controls",children:[t.jsx("button",{className:"btn btn-sm",onClick:()=>g(v*.8),children:"Zoom Out"}),t.jsxs("span",{className:"zoom-level",children:[Math.round(v*100),"%"]}),t.jsx("button",{className:"btn btn-sm",onClick:()=>g(v*1.2),children:"Zoom In"})]}),t.jsx("div",{className:"canvas-size",children:t.jsxs("select",{value:`${m.width}x${m.height}`,onChange:D=>{const[I,F]=D.target.value.split("x").map(Number);x({width:I,height:F})},children:[t.jsx("option",{value:"1200x800",children:"Desktop (1200x800)"}),t.jsx("option",{value:"768x1024",children:"Tablet (768x1024)"}),t.jsx("option",{value:"375x667",children:"Mobile (375x667)"})]})})]}),t.jsx("div",{className:"canvas",ref:f,style:{width:m.width*v,height:m.height*v,transform:`scale(${v})`,transformOrigin:"top left"},children:t.jsx(lg,{onDrop:j,children:n.map(D=>t.jsx(cg,{component:D,onSelect:w,isSelected:(i==null?void 0:i.id)===D.id},D.id))})})]}),l&&i&&t.jsxs("div",{className:"properties-panel",children:[t.jsx("h3",{children:"⚙️ Properties"}),t.jsxs("div",{className:"property-groups",children:[t.jsxs("div",{className:"property-group",children:[t.jsx("h4",{children:"Content"}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"Text Content"}),t.jsx("input",{type:"text",value:i.content||"",onChange:D=>y("content",D.target.value)})]})]}),t.jsxs("div",{className:"property-group",children:[t.jsx("h4",{children:"Position"}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"X Position"}),t.jsx("input",{type:"number",value:((k=i.position)==null?void 0:k.x)||0,onChange:D=>y("left",`${D.target.value}px`)})]}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"Y Position"}),t.jsx("input",{type:"number",value:((L=i.position)==null?void 0:L.y)||0,onChange:D=>y("top",`${D.target.value}px`)})]})]}),t.jsxs("div",{className:"property-group",children:[t.jsx("h4",{children:"Size"}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"Width"}),t.jsx("input",{type:"number",value:((E=i.size)==null?void 0:E.width)||200,onChange:D=>y("width",`${D.target.value}px`)})]}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"Height"}),t.jsx("input",{type:"number",value:((P=i.size)==null?void 0:P.height)||100,onChange:D=>y("height",`${D.target.value}px`)})]})]}),t.jsxs("div",{className:"property-group",children:[t.jsx("h4",{children:"Style"}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"Background Color"}),t.jsx("input",{type:"color",value:((Y=i.styles)==null?void 0:Y.backgroundColor)||"#ffffff",onChange:D=>y("backgroundColor",D.target.value)})]}),t.jsxs("div",{className:"property",children:[t.jsx("label",{children:"Border Color"}),t.jsx("input",{type:"color",value:((H=i.styles)==null?void 0:H.borderColor)||"#dddddd",onChange:D=>y("borderColor",D.target.value)})]})]})]})]})]}),t.jsx("style",{jsx:!0,children:`
          .visual-editor {
            height: 100vh;
            display: flex;
            flex-direction: column;
