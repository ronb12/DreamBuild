...
import { detectSupportedLanguage, getLanguageFileExtension, getLanguageLabel } from '../data/supportedLanguages.js'

class DreamBuildAI {
  constructor() {
    this.engineVersion = 'prompt-aware-v6-autonomous'
  ...
  generateCSSTemplate(appType, features) {
    if (appType === 'game') {
      return this.generateGameCSS({ appType, features })
    }
...
.app-header {
  background:
    radial-gradient(circle at 12% 10%, rgba(6, 182, 212, 0.24), transparent 20rem),
...
self.addEventListener('fetch', (event) => {
  ...
  toSafeIdentifier(value, fallback = 'DreamBuildApp') {
    const cleaned = this.toTitleCase(value)
      .replace(/[^a-zA-Z0-9 ]/g, '')
      .replace(/\s+/g, '')
    return cleaned && /^[A-Za-z]/.test(cleaned) ? cleaned : fallback
  }

  deriveAppNameFromPrompt(prompt, appType) {
    const lowerPrompt = String(prompt || '').toLowerCase()
    const originalPrompt = String(prompt || '').trim()
    const explicitNameMatch = originalPrompt.match(/\b(?:called|named|titled)\s+["']?([^"'.!?]+?)["']?(?:\.|,|;|!|\?|$)/i)

    if (explicitNameMatch?.[1]) {
      const explicitName = explicitNameMatch[1]
        .replace(/\b(?:that|with|and|it|should|build|create)\b.*$/i, '')
        .replace(/\s+/g, ' ')
        .trim()

      if (explicitName.length >= 3) {
        return this.toTitleCase(explicitName)
      }
    }

    const appTypeNames = {
      ecommerce: 'Commerce Studio',
      travel: 'Travel Booking Hub',
      hotel: 'Hotel Reservation Hub',
      fitness: 'Fitness Progress Tracker',
      medical: 'Health Care Dashboard',
      crm: 'Customer Pipeline CRM',
      dashboard: 'Operations Dashboard',
      calendar: 'Smart Scheduling Calendar',
      restaurant: 'Restaurant Order Manager',
      ...
      api: 'API Service Builder'
    }

    const phraseNames = [
      { test: ['travel', 'booking'], name: 'Travel Booking Hub' },
      { test: ['hotel', 'flight'], name: 'Travel Listings Marketplace' },
      { test: ['car rental'], name: 'Car Rental Booking Hub' },
      { test: ['admin dashboard'], name: `${this.toTitleCase(appType)} Admin Dashboard` },
      { test: ['checkout', 'payment'], name: `${this.toTitleCase(appType)} Checkout Portal` },
      { test: ['fitness', 'tracker'], name: 'Fitness Progress Tracker' },
      { test: ['restaurant', 'order'], name: 'Restaurant Order Manager' },
      { test: ['portfolio', 'gallery'], name: 'Portfolio Gallery' }
    ]

    const phraseMatch = phraseNames.find(({ test }) => test.every(term => lowerPrompt.includes(term)))
    if (phraseMatch) return phraseMatch.name

    return appTypeNames[appType] || `${this.toTitleCase(appType)} App`
  }
  ...
  buildPromptAwareSpec(appType, features = [], intent = {}) {
    const prompt = String(intent.originalPrompt || '').trim()
    const lowerPrompt = prompt.toLowerCase()
    const appTitle = intent.appName || this.toTitleCase(appType)
    ...
    const domainCatalog = {
      ecommerce: {
        ...
        plural: 'products',
        action: 'Add Product',
        hero: 'A storefront workspace with catalog, cart, checkout readiness, and order tracking.',
        fields: ['Product name', 'Category', 'Price', 'Inventory'],
        metrics: ['Revenue', 'Orders', 'Cart Items', 'Conversion'],
        sample: ['Signature Hoodie', 'Digital Template', 'Starter Kit']
      },
      travel: {
        entity: 'trip',
        plural: 'trips',
        action: 'Schedule Trip',
        hero: 'A travel planning hub for itineraries, bookings, budgets, and traveler notes.',
        fields: ['Destination', 'Dates', 'Budget', 'Travelers'],
        metrics: ['Trips', 'Booked Items', 'Budget Used', 'Tasks'],
        sample: ['Miami Weekend', 'Family Cruise', 'Business Flight']
      },
      hotel: {
        entity: 'reservation',
        plural: 'reservations',
        action: 'Add Reservation',
        hero: 'A booking dashboard for rooms, guests, rates, and reservation status.',
        fields: ['Guest name', 'Room type', 'Check-in', 'Rate'],
        metrics: ['Reservations', 'Available Rooms', 'Revenue', 'Guests'],
        sample: ['King Suite', 'Ocean View', 'Conference Block']
      },
      fitness: {
        entity: 'workout',
        plural: 'workouts',
        action: 'Log Workout',
        hero: 'A fitness tracker for training plans, progress, goals, and wellness habits.',
        fields: ['Workout', 'Duration', 'Intensity', 'Goal'],
        metrics: ['Workouts', 'Calories', 'Streak', 'Goals'],
        sample: ['Upper Body', 'Cardio Day', 'Mobility Flow']
      },
      medical: {
        entity: 'health record',
        plural: 'health records',
        action: 'Add Record',
        hero: 'A health workspace for patient notes, reminders, vitals, and care tasks.',
        fields: ['Record name', 'Category', 'Date', 'Status'],
        metrics: ['Records', 'Reminders', 'Vitals', 'Care Tasks'],
        sample: ['Blood Pressure', 'Medication Reminder', 'Follow-up Visit']
      },
      crm: {
        entity: 'lead',
        plural: 'leads',
        action: 'Add Lead',
        hero: 'A customer relationship dashboard for leads, deals, follow-ups, and pipeline health.',
        fields: ['Lead name', 'Company', 'Deal value', 'Stage'],
        metrics: ['Leads', 'Deals', 'Pipeline', 'Follow-ups'],
        sample: ['Acme Corp', 'New Prospect', 'Renewal Account']
      },
      dashboard: {
        entity: 'metric',
        plural: 'metrics',
        action: 'Add Metric',
        hero: 'A command dashboard for KPIs, alerts, reports, and operational workflows.',
        fields: ['Metric name', 'Category', 'Value', 'Owner'],
        metrics: ['KPIs', 'Alerts', 'Reports', 'Tasks'],
        sample: ['Revenue Trend', 'Open Alerts', 'Weekly Report']
      },
      calendar: {
        entity: 'event',
        plural: 'events',
        action: 'Schedule Event',
        hero: 'A scheduling workspace for events, appointments, reminders, and availability.',
        fields: ['Event name', 'Date', 'Time', 'Attendees'],
        metrics: ['Events', 'Reminders', 'Open Slots', 'Attendees'],
        sample: ['Client Call', 'Team Meeting', 'Launch Date']
      },
      restaurant: {
        entity: 'order',
        plural: 'orders',
        action: 'Add Order',
        hero: 'A restaurant operating screen for menu items, orders, tables, and delivery flow.',
        fields: ['Order name', 'Menu item', 'Table', 'Status'],
        metrics: ['Orders', 'Tables', 'Revenue', 'Prep Queue'],
        sample: ['Lunch Combo', 'Family Meal', 'Delivery Order']
      },
      portfolio: {
        entity: 'project',
        plural: 'projects',
        action: 'Add Project',
        hero: 'A portfolio showcase for case studies, galleries, testimonials, and project leads.',
        fields: ['Project name', 'Category', 'Client', 'Status'],
        metrics: ['Projects', 'Views', 'Inquiries', 'Testimonials'],
        sample: ['Brand Refresh', 'Website Launch', 'Photo Gallery']
      }
    }

    const spec = domainCatalog[appType] || {
      entity: this.toTitleCase(appType).toLowerCase(),
      plural: `${this.toTitleCase(appType).toLowerCase()} items`,
      action: `Add ${this.toTitleCase(appType)} Item`,
      hero: `A purpose-built ${this.toTitleCase(appType)} app generated directly from the user's request.`,
      fields: ['Name', 'Category', 'Priority', 'Status'],
      metrics: ['Items', 'Active', 'Completed', 'Needs Review'],
      sample: ['Primary Item', 'Follow-up Item', 'Review Item']
    }

    if (lowerPrompt.includes('booking') || lowerPrompt.includes('reservation')) {
      spec.fields = [...new Set([...spec.fields, 'Booking Date', 'Confirmation'])]
      featureSet.push('booking')
    }
    if (lowerPrompt.includes('payment') || lowerPrompt.includes('checkout') || lowerPrompt.includes('stripe')) {
      spec.fields = [...new Set([...spec.fields, 'Payment Status'])]
      featureSet.push('checkout')
    }
    if (lowerPrompt.includes('admin')) featureSet.push('admin-dashboard')
    if (lowerPrompt.includes('email') || lowerPrompt.includes('notification')) featureSet.push('notifications')
    if (lowerPrompt.includes('upload') || lowerPrompt.includes('image') || lowerPrompt.includes('photo')) featureSet.push('media-upload')
    if (lowerPrompt.includes('map') || lowerPrompt.includes('location')) featureSet.push('location')
    ...
    return {
      ...
      action: spec.action,
      hero: spec.hero,
      fields: spec.fields.slice(0, 6),
      metrics: spec.metrics,
      sample: spec.sample,
      features: [...new Set(featureSet)].slice(0, 14)
    }
  }
  ...
  generateGenericJS(appType, features, intent) {
    const spec = this.buildPromptAwareSpec(appType, features, intent)
    const className = this.toSafeIdentifier(appType)

    return `// ${spec.appTitle} - Generated by DreamBuild AI
// Product of Bradley Virtual Solutions, LLC

console.log('🚀 Initializing ${spec.appTitle}...');

class ${className}App {
  constructor() {
    this.storageKey = 'dreambuild-${appType}-records';
  ...
  init() {
    ...
    this.updateMetrics();
    this.bindEvents();
    console.log('✅ ${spec.appTitle} initialized');
  }

  loadRecords() {
    const saved = localStorage.getItem(this.storageKey);
    if (saved) return JSON.parse(saved);
    return ${JSON.stringify(spec.sample.map((item, index) => ({
      id: index + 1,
      name: item,
      category: spec.fields[1] || 'General',
      status: index === 0 ? 'Active' : index === 1 ? 'Pending' : 'Ready',
      value: index === 0 ? '$1,250' : index === 1 ? '$840' : '$420'
    })), null, 4)};
  }
  ...
  bindEvents() {
    document.getElementById('record-form')?.addEventListener('submit', (event) => {
      ...
      const record = {
        id: Date.now(),
        name: formData.get('name') || '${this.escapeHTML(spec.entity)}',
        category: formData.get('category') || 'General',
        status: formData.get('status') || 'Active',
        value: formData.get('value') || 'New'
      };
    ...
    document.getElementById('export-btn')?.addEventListener('click', () => {
      ...
      link.href = url;
      link.download = '${appType}-records.json';
      link.click();
  ...
  renderRecords(query = '') {
    ...
    const visibleRecords = this.records.filter(record => {
      return Object.values(record).join(' ').toLowerCase().includes(normalizedQuery);
    });
    ...
    \`).join('') || '<p class="empty-state">No ${spec.plural} match your search yet.</p>';
  }

  updateMetrics() {
    document.getElementById('metric-total').textContent = this.records.length;
    document.getElementById('metric-active').textContent = this.records.filter(record => record.status === 'Active').length;
    document.getElementById('metric-ready').textContent = this.records.filter(record => record.status === 'Ready').length;
    document.getElementById('metric-pending').textContent = this.records.filter(record => record.status === 'Pending').length;
  }
}
...
if (document.readyState === 'loading') {
  ...
  shouldUseLanguageScaffold(language, prompt) {
    if (!language || ['html', 'css', 'javascript'].includes(language)) return false
    const lowerPrompt = String(prompt || '').toLowerCase()
    return [
      ' in ',
      ' using ',
  ...
  createComment(language, text) {
    if (['html', 'xml'].includes(language)) return `<!-- ${text} -->`
    if (['css'].includes(language)) return `/* ${text} */`
    if (['bash', 'powershell', 'python', 'ruby', 'perl', 'r', 'julia', 'yaml', 'dockerfile', 'makefile'].includes(language)) return `# ${text}`
    if (['sql', 'lua', 'haskell', 'erlang'].includes(language)) return `-- ${text}`
    return `// ${text}`
  }

  generateLanguageScaffold(prompt, intent, language) {
    const label = getLanguageLabel(language)
    const extension = getLanguageFileExtension(language)
    const appName = intent.appName || 'DreamBuild App'
    const mainFile = extension === 'Dockerfile' ? 'Dockerfile' : `main.${extension}`
    const comment = this.createComment(language, `${appName} generated by DreamBuild AI`)
    const safePrompt = String(prompt || '').replace(/`/g, "'")

    const sourceByLanguage = {
      json: JSON.stringify({ name: appName, description: safePrompt, generatedBy: 'DreamBuild AI' }, null, 2),
      yaml: `name: ${appName}\ndescription: ${safePrompt}\ngeneratedBy: DreamBuild AI\n`,
      xml: `<?xml version="1.0" encoding="UTF-8"?>\n<app name="${appName}">\n  <description>${safePrompt}</description>\n  <generatedBy>DreamBuild AI</generatedBy>\n</app>\n`,
      markdown: `# ${appName}\n\nGenerated by DreamBuild AI.\n\n## Request\n\n${safePrompt}\n\n## Next Steps\n\n- Add production logic.\n- Run this project in a ${label} environment.\n- Connect databases, APIs, and deployment credentials as needed.\n`,
      sql: `${comment}\nCREATE TABLE app_items (\n  id INTEGER PRIMARY KEY,\n  title VARCHAR(255) NOT NULL,\n  status VARCHAR(50) DEFAULT 'new',\n  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP\n);\n\nINSERT INTO app_items (id, title, status) VALUES (1, '${appName}', 'generated');\n`,
      dockerfile: `FROM alpine:latest\nLABEL app="${appName}"\nCMD [\"echo\", \"${appName} generated by DreamBuild AI\"]\n`,
      makefile: `${comment}\nrun:\n\t@echo \"${appName} generated by DreamBuild AI\"\n`
    }

    const genericSource = `${comment}\n\n${this.createComment(language, `Request: ${safePrompt}`)}\n\n${this.createComment(language, `This is a ${label} starter scaffold. Add framework-specific packages, secrets, and deployment settings as needed.`)}\n\n${this.createComment(language, 'Entry point')}\n`

    return {
      [mainFile]: sourceByLanguage[language] || genericSource,
      'README.md': `# ${appName}\n\nDreamBuild generated a ${label} starter scaffold for this request:\n\n> ${safePrompt}\n\n## Language Coverage\n\nThis project was created from DreamBuild's 61-language support path. Web projects can preview immediately in the browser. Native, backend, mobile, database, and infrastructure projects can be queued from inside DreamBuild through the DreamBuild Cloud Runner, which handles the required compilers, signing credentials, and deployment toolchains when connected.\n`,
      'dreambuild.json': JSON.stringify({
        name: appName,
        prompt: safePrompt,
  ...
  runAutonomousBuildPass(files, intent = {}, context = {}) {
    console.log('🤖 DreamBuild autonomous build pass starting...')

    const repairedFiles = { ...(files || {}) }
    const repairs = []
    const prompt = String(intent.originalPrompt || context.prompt || '').trim()

    const hasFailureShell = Object.values(repairedFiles).some((content) => {
      return typeof content === 'string' && content.includes('Generation Failed')
    })

    if (!repairedFiles['index.html'] || hasFailureShell) {
      repairedFiles['index.html'] = this.generatePageTemplate(
        intent.appType || 'web-app',
        intent.technology || 'vanilla',
        intent.features || [],
        intent
      )
      repairs.push(hasFailureShell ? 'Replaced failed HTML shell' : 'Created missing index.html')
    }

    if (!repairedFiles['styles.css']) {
      repairedFiles['styles.css'] = this.generateCSSTemplate(intent.appType || 'web-app', intent.features || [])
      repairs.push('Created missing styles.css')
    }

    if (!repairedFiles['script.js']) {
      repairedFiles['script.js'] = this.generateJSTemplate(intent.appType || 'web-app', intent.features || [], intent)
      repairs.push('Created missing script.js')
    }

    const html = repairedFiles['index.html'] || ''
    const expectedName = intent.appName || this.deriveAppNameFromPrompt(prompt, intent.appType || 'web-app')
    if (expectedName && !html.includes(expectedName)) {
      repairedFiles['index.html'] = html
        .replace(/<title>.*?<\/title>/i, `<title>${this.escapeHTML(expectedName)}</title>`)
        .replace(/<h1([^>]*)>.*?<\/h1>/i, `<h1$1>${intent.appEmoji || '✨'} ${this.escapeHTML(expectedName)}</h1>`)
      repairs.push(`Aligned visible title with "${expectedName}"`)
    }
    ...
    const missingSections = requestedSections
      .filter(([keyword]) => !repairedFiles['index.html'].toLowerCase().includes(keyword))
      .map(([, label]) => label)

    if (missingSections.length > 0) {
      repairedFiles['index.html'] = repairedFiles['index.html'].replace('</main>', `
        <section class="content-card full-width">
          <h2>Autonomous Completion Pass</h2>
          <p>DreamBuild added missing requested sections during validation.</p>
          <div class="feature-grid">
            ${missingSections.map((section) => `<span class="feature-chip">${this.escapeHTML(section)}</span>`).join('')}
          </div>
        </section>
      </main>`)
      repairs.push(`Added missing requested sections: ${missingSections.join(', ')}`)
    }

    const checks = {
      hasHTML: Boolean(repairedFiles['index.html']),
      hasCSS: Boolean(repairedFiles['styles.css']),
      hasJS: Boolean(repairedFiles['script.js']),
      noFailureShell: !Object.values(repairedFiles).some((content) => typeof content === 'string' && content.includes('Generation Failed')),
      preservedRequestedName: expectedName ? repairedFiles['index.html'].includes(expectedName) : true,
      requestedSectionsCovered: true
    }

    console.log('✅ DreamBuild autonomous build pass complete:', { checks, repairs })
    ...
    return {
      ...
      report: {
        ...
        checks,
        repairs,
        status: Object.values(checks).every(Boolean) ? 'passed' : 'repaired-with-warnings',
        runnerRequiredFor: [
          'real terminal commands',
          'compiled native/mobile/backend execution',
...
<body>
  <div id="app">
    ...
    if (appType === 'game') {
      const gameTitle = this.escapeHTML(intent.appName || 'Rainbow Rocket Rescue')
      const promptSummary = this.escapeHTML(intent.originalPrompt || 'A kid-friendly browser game.')
...
<head>
  ...
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Baloo+2:wght@500;700;800&family=Inter:wght@400;600;800&display=swap" rel="stylesheet">
</head>
<body>
  <div id="app" class="kids-game-app">
    <header class="game-header">
      <div class="container">
        <h1 class="game-title">🚀 ${gameTitle}</h1>
        <p class="game-overlay-text">Collect friendly stars, dodge sleepy clouds, and rescue the rainbow.</p>
        <p class="request-summary">Built from request: ${promptSummary}</p>
        <div class="game-stats" aria-label="Game stats">
          <div class="stat-item"><span class="stat-label">Score</span><strong id="score" class="stat-value">0</strong></div>
          <div class="stat-item"><span class="stat-label">Lives</span><strong id="lives" class="stat-value">3</strong></div>
          <div class="stat-item"><span class="stat-label">Level</span><strong id="level" class="stat-value">1</strong></div>
        </div>
      </div>
    </header>
    ...
    <main class="game-main">
      <div class="container">
        <section class="game-container" aria-label="${gameTitle} playable game">
          <canvas id="game-canvas" width="900" height="520" aria-label="Rainbow Rocket Rescue game canvas"></canvas>
          <div id="game-overlay" class="game-overlay">
            <h2 class="game-overlay-title">Ready, Pilot?</h2>
            <p class="game-overlay-text">Use arrow keys or the touch controls to collect stars and avoid sleepy clouds.</p>
            <button id="start-btn" class="btn btn-primary btn-large" type="button">Start Game</button>
          </div>
          <div id="game-over" class="game-overlay" style="display: none;">
            <h2 class="game-overlay-title">Great Flying!</h2>
            <p class="game-overlay-text">Final score: <strong id="final-score">0</strong></p>
            <button id="restart-btn" class="btn btn-primary btn-large" type="button">Play Again</button>
          </div>
        </section>

        <section class="game-controls" aria-label="Game controls">
          <div class="controls-grid">
            <button id="left-btn" class="control-item" type="button">⬅️ Left</button>
            <button id="up-btn" class="control-item" type="button">⬆️ Up</button>
            <button id="down-btn" class="control-item" type="button">⬇️ Down</button>
            <button id="right-btn" class="control-item" type="button">➡️ Right</button>
            <button id="pause-btn" class="control-item" type="button">⏸️ Pause</button>
            <button id="sound-toggle" class="control-item" type="button">🔊 Sound On</button>
          </div>
        </section>
      </div>
    </main>
    ...
    <footer class="game-footer">
      <div class="container">
        <p>Safe kid-friendly game. No ads, no scary content, no data collection. Built with DreamBuild.</p>
      </div>
    </footer>
  </div>
</body>
    ...
    // Prompt-aware "from scratch" template - builds a usable app for the requested domain.
    console.log('🎨 Generating prompt-aware app for:', appType, 'with', features.length, 'features')

    const spec = this.buildPromptAwareSpec(appType, features, intent)
    const appTitle = this.escapeHTML(spec.appTitle)
    const hasDarkMode = spec.features.includes('darkMode') || features.includes('darkMode')
    const hasExport = spec.features.includes('export') || features.includes('export') || true
    const hasSearch = spec.features.includes('search') || features.includes('search') || true
    const promptSummary = this.escapeHTML(spec.prompt || spec.hero)
...
<body>
  <div id="app">
    <header class="app-header">
      <div class="container">
        <div class="hero-layout">
          <div>
            <p class="eyebrow">Built from your request</p>
            <h1 class="app-title">✨ ${appTitle}</h1>
            <p class="app-subtitle">${this.escapeHTML(spec.hero)}</p>
            <p class="request-summary">Request: ${promptSummary}</p>
          </div>
          <div class="header-actions">
            ${hasDarkMode ? '<button id="theme-toggle" class="btn btn-secondary">🌙 Dark Mode</button>' : ''}
            ${hasExport ? '<button id="export-btn" class="btn btn-secondary">📥 Export</button>' : ''}
            <span class="feature-badge">✨ ${spec.features.length || 4} Features</span>
          </div>
        </div>
      </div>
    </header>
      ...
      <div class="container">
        <section class="metrics-grid">
          ...
          <div class="metric-card">
            <span>${this.escapeHTML(metric)}</span>
            <strong id="${['metric-total', 'metric-active', 'metric-ready', 'metric-pending'][index] || `metric-${index}`}">${index === 0 ? spec.sample.length : index}</strong>
          </div>
          `).join('')}
        </section>
        ...
        <section class="workspace-grid">
          <div class="content-card">
            <h2>${this.escapeHTML(spec.action)}</h2>
            <p>Create and track ${this.escapeHTML(spec.plural)} directly in this generated app.</p>
            <form id="record-form" class="record-form">
              <input name="name" class="input" placeholder="${this.escapeHTML(spec.fields[0] || 'Name')}" required>
              <input name="category" class="input" placeholder="${this.escapeHTML(spec.fields[1] || 'Category')}">
              <input name="value" class="input" placeholder="${this.escapeHTML(spec.fields[2] || 'Value')}">
              ...
              <button class="btn btn-primary" type="submit">${this.escapeHTML(spec.action)}</button>
            </form>
          </div>
          ...
          <div class="content-card">
            <div class="section-heading">
              <div>
                <h2>${this.toTitleCase(spec.plural)}</h2>
                <p>Search, review, and manage generated sample data.</p>
              </div>
              ${hasSearch ? '<input id="search-input" class="input compact-input" placeholder="Search records...">' : ''}
            </div>
          ...
          <div class="content-card full-width">
            ...
            <div class="feature-grid">
              ${(spec.features.length ? spec.features : ['records', 'dashboard', 'search', 'export']).map(feature => `
                <span class="feature-chip">${this.escapeHTML(this.toTitleCase(feature))}</span>
              `).join('')}
            </div>
            <div class="workflow-list">
              <span>1. Capture ${this.escapeHTML(spec.entity)} details</span>
              <span>2. Track status and value</span>
              <span>3. Search active records</span>
              <span>4. Export the data for follow-up</span>
            </div>
          </div>
        </section>
      </div>
    ...
    <footer class="app-footer">
      <div class="container">
        <p>&copy; 2024 ${this.currentAppName || appTitle}. Built with DreamBuild - Product of Bradley Virtual Solutions, LLC</p>
      </div>
    </footer>
  </div>
  ...
  async generateCode(prompt, context = {}) {
    ...
    try {
      if (promptCacheService && typeof promptCacheService.checkCache === 'function') {
        ...
        if (cacheResult.found && cacheResult.response?.metadata?.engineVersion === this.engineVersion) {
        ...
        if (cacheResult.found) {
          console.log('🧹 Ignoring stale Firebase cache from older build engine')
        } else {
    ...
    try {
      ...
      intent.appName = this.deriveAppNameFromPrompt(prompt, intent.appType) || (typeof nameData === 'string' ? nameData : nameData.name)
      intent.appEmoji = typeof nameData === 'object' ? nameData.emoji : '✨'
      intent.originalPrompt = prompt
      intent.targetLanguage = detectSupportedLanguage(prompt, null)
      
      console.log('🎯 Intent analyzed:', intent)
      console.log('✨ App name:', intent.appName, intent.appEmoji)

      if (this.shouldUseLanguageScaffold(intent.targetLanguage, prompt)) {
        const languageFiles = this.generateLanguageScaffold(prompt, intent, intent.targetLanguage)
        return {
          files: languageFiles,
          type: 'language_scaffold_generated',
          message: `Generated a ${getLanguageLabel(intent.targetLanguage)} starter scaffold.`,
          metadata: {
            intent,
            aiEngine: 'DreamBuild Built-in AI',
            version: '1.0.0',
      ...
      // Step 5: Autonomous validation and repair before files reach the editor
      const autonomousPass = this.runAutonomousBuildPass(enhancedFiles, intent, {
        ...context,
        prompt
      })
      
      const result = {
        files: autonomousPass.files,
        type: 'built_in_ai_generated',
          metadata: {
            intent,
            structure,
            autonomousPass: autonomousPass.report,
...
`
  ...
  generateGameJS(intent) {
    const prompt = intent.originalPrompt || 'Rainbow Rocket Rescue kids game'
    const appName = intent.appName || 'Rainbow Rocket Rescue'
    const lowerPrompt = prompt.toLowerCase()

    if (lowerPrompt.includes('tetris') || lowerPrompt.includes('block')) {
      return this.generateTetrisGame(prompt, appName)
    }

    if (lowerPrompt.includes('snake')) {
      return this.generateSnakeGame(prompt, appName)
    }

    if (lowerPrompt.includes('pong')) {
      return this.generatePongGame(prompt, appName)
    }

    if (lowerPrompt.includes('breakout') || lowerPrompt.includes('brick')) {
      return this.generateBreakoutGame(prompt, appName)
    }

    if (lowerPrompt.includes('flappy')) {
      return this.generateFlappyBirdGame(prompt, appName)
    }

    if (lowerPrompt.includes('space invader') || lowerPrompt.includes('alien')) {
      return this.generateSpaceInvadersGame(prompt, appName)
    }

    return `// ${appName} - Generated by DreamBuild
// Product of Bradley Virtual Solutions, LLC
console.log('🚀 Initializing ${appName}...')

class RainbowRocketRescue {
  constructor() {
    this.canvas = document.getElementById('game-canvas')
    this.ctx = this.canvas.getContext('2d')
    this.score = 0
    ...
    this.spawnTimer = 0
    this.starTimer = 0
    this.player = { x: 80, y: 230, width: 58, height: 42, speed: 340 }
    this.stars = []
    this.clouds = []
    this.sparkles = []
    this.bindControls()
  ...
  resize() {
    ...
    const width = Math.min(900, Math.max(320, shell.clientWidth - 40))
    this.canvas.style.width = width + 'px'
    this.canvas.style.height = Math.round(width * 0.58) + 'px'
  }
  ...
  bindControls() {
    document.addEventListener('keydown', (event) => {
      ...
      if (event.key === ' ' && !this.running) this.start()
    ...
    const hold = (id, key) => {
      ...
      if (!button) return
      const down = (event) => { event.preventDefault(); this.keys[key] = true }
      const up = (event) => { event.preventDefault(); this.keys[key] = false }
    ...
    hold('left-btn', 'ArrowLeft')
    hold('right-btn', 'ArrowRight')
    hold('up-btn', 'ArrowUp')
    hold('down-btn', 'ArrowDown')
  ...
  toggleSound() {
    ...
    document.getElementById('sound-toggle').textContent = this.soundOn ? '🔊 Sound On' : '🔇 Sound Off'
  }

  beep(frequency = 660, duration = 0.06) {
    if (!this.soundOn || !window.AudioContext) return
  ...
  loop(time) {
    ...
    const dt = Math.min(0.032, (time - this.lastFrame) / 1000 || 0.016)
    this.lastFrame = time
    this.update(dt)
    this.draw()
  ...
  update(dt) {
    const speed = this.player.speed + this.level * 16
    ...
    this.player.y = Math.max(8, Math.min(this.canvas.height - this.player.height - 8, this.player.y))

    this.spawnTimer += dt
    this.starTimer += dt
    if (this.spawnTimer > Math.max(0.72, 1.6 - this.level * 0.12)) {
      this.spawnTimer = 0
      this.clouds.push(this.makeCloud())
    }
    if (this.starTimer > Math.max(0.42, 0.9 - this.level * 0.04)) {
      this.starTimer = 0
      this.stars.push(this.makeStar())
    }

    this.clouds.forEach((cloud) => cloud.x -= cloud.speed * dt)
    this.stars.forEach((star) => {
      star.x -= star.speed * dt
      star.spin += dt * 5
    })
    this.sparkles.forEach((sparkle) => sparkle.life -= dt)
    this.clouds = this.clouds.filter((cloud) => cloud.x > -90)
    this.stars = this.stars.filter((star) => star.x > -40 && !star.collected)
    this.sparkles = this.sparkles.filter((sparkle) => sparkle.life > 0)

    this.stars.forEach((star) => {
      if (!star.collected && this.overlaps(this.player, star)) {
        star.collected = true
        this.score += 10
        this.level = Math.floor(this.score / 80) + 1
        this.addSparkles(star.x, star.y, '#fde047')
        this.beep(820, 0.05)
        this.updateHud()
      }
    })
    ...
    this.clouds.forEach((cloud) => {
      if (!cloud.hit && this.overlaps(this.player, cloud)) {
        cloud.hit = true
        this.lives -= 1
        this.addSparkles(this.player.x + 24, this.player.y + 18, '#93c5fd')
        this.beep(180, 0.08)
        this.updateHud()
  ...
  makeCloud() {
    return {
      ...
      y: 42 + Math.random() * (this.canvas.height - 96),
      width: 72,
      height: 44,
      speed: 135 + this.level * 22,
      hit: false
    }
  }
  ...
  drawWelcome() {
    ...
    this.ctx.textAlign = 'center'
    this.ctx.fillText('Rainbow Rocket Rescue', this.canvas.width / 2, this.canvas.height / 2 - 16)
    this.ctx.font = '600 18px Inter, sans-serif'
    this.ctx.fillText('Press Start to play', this.canvas.width / 2, this.canvas.height / 2 + 24)
  }
  ...
  drawPause() {
    ...
    this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height)
    this.ctx.fillStyle = '#fff'
    this.ctx.font = '800 40px Inter, sans-serif'
    this.ctx.textAlign = 'center'
  ...
  drawBackground() {
    ...
    for (let i = 0; i < 7; i += 1) {
      ...
      this.ctx.beginPath()
      this.ctx.arc(80 + i * 130, 70 + Math.sin(i) * 26, 38, 0, Math.PI * 2)
      this.ctx.fill()
    }
  }
  ...
  drawRocket() {
    ...
    this.ctx.fillStyle = '#ffffff'
    this.ctx.beginPath()
    this.ctx.moveTo(58, 21)
    this.ctx.lineTo(12, 4)
    this.ctx.quadraticCurveTo(0, 21, 12, 38)
    this.ctx.closePath()
    this.ctx.fill()
    this.ctx.fillStyle = '#2563eb'
    this.ctx.fillRect(14, 12, 24, 18)
    this.ctx.fillStyle = '#f97316'
    this.ctx.beginPath()
    this.ctx.moveTo(5, 15)
    this.ctx.lineTo(-18, 21)
    this.ctx.lineTo(5, 28)
    this.ctx.fill()
    this.ctx.fillStyle = '#67e8f9'
    this.ctx.beginPath()
    this.ctx.arc(43, 20, 7, 0, Math.PI * 2)
    this.ctx.fill()
    this.ctx.restore()
  }
  ...
  drawStar(star) {
    this.ctx.save()
    this.ctx.translate(star.x + 14, star.y + 14)
  ...
  drawCloud(cloud) {
    this.ctx.fillStyle = cloud.hit ? 'rgba(147,197,253,0.35)' : 'rgba(255,255,255,0.88)'
    this.ctx.beginPath()
    this.ctx.arc(cloud.x + 18, cloud.y + 24, 18, 0, Math.PI * 2)
    this.ctx.arc(cloud.x + 38, cloud.y + 16, 23, 0, Math.PI * 2)
    this.ctx.arc(cloud.x + 58, cloud.y + 25, 17, 0, Math.PI * 2)
    this.ctx.fill()
    this.ctx.fillStyle = '#64748b'
    this.ctx.fillRect(cloud.x + 24, cloud.y + 25, 7, 3)
    this.ctx.fillRect(cloud.x + 48, cloud.y + 25, 7, 3)
  }
  ...
  draw() {
    ...
    this.sparkles.forEach((sparkle) => {
      this.ctx.globalAlpha = Math.max(0, sparkle.life)
      this.ctx.fillStyle = sparkle.color
      this.ctx.beginPath()
      this.ctx.arc(sparkle.x, sparkle.y, 4, 0, Math.PI * 2)
      this.ctx.fill()
...
function bootGame() {
  window.rainbowRocketRescue = new RainbowRocketRescue()
}
