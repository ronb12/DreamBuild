...
async function testAIAssistantFixSimple() {
  ...
  try {
    ...
    const aiAssistantCheck = await page.evaluate(() => {
      ...
      for (let i = 0; i < allElements.length; i++) {
        if (allElements[i].textContent && allElements[i].textContent.includes('AI Assistant')) {
          aiHeaders++;
        }
      }
      ...
      return {
        ...
        aiHeaders: aiHeaders,
        hasDuplication: promptInputs.length > 1 || sendButtons > 1
      };
    });
    ...
    const aiTabTest = await page.evaluate(() => {
      ...
      for (let i = 0; i < tabs.length; i++) {
        if (tabs[i].textContent && (tabs[i].textContent.includes('AI Assistant') || tabs[i].textContent.includes('AI Chat'))) {
          aiTab = tabs[i];
          break;
        }
      }
    ...
    const aiFunctionalityCheck = await page.evaluate(() => {
      ...
      return {
        ...
        generateButtons: generateButtons.length,
        sendButtons: sendButtons,
        isWorking: promptInputs.length === 1 && sendButtons === 1
      };
    });
    
    console.log('AI Functionality Check:', aiFunctionalityCheck);
    
    // Take screenshot
    await page.screenshot({ 
      path: 'ai-assistant-fix-simple-test.png',
      fullPage: true 
    });
    
    console.log('📸 Screenshot saved: ai-assistant-fix-simple-test.png');
...
testAIAssistantFixSimple().then(result => {
  if (result.error) {
