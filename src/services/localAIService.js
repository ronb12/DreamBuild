...
const SUPPORTED_LANGUAGE_VALUES = supportedLanguages.map((language) => language.value)
const STARTER_TEMPLATE_COUNT = 300
const GAME_TEMPLATE_COUNT = 300
const FEATURE_TEMPLATE_COUNT = 300
const DESIGN_TEMPLATE_COUNT = 5000
const ASSET_PREVIEW_TEMPLATE_COUNT = 300
const APP_ICON_GENERATOR_COUNT = 40000
const FAVICON_GENERATOR_COUNT = 40000
const IMAGE_GENERATOR_COUNT = 40000

const buildGeneratedTemplates = (count, prefix, category, nameParts, descriptorParts, files, extraTags = []) => {
  return Array.from({ length: count }, (_, index) => {
    const namePart = nameParts[index % nameParts.length]
    const descriptor = descriptorParts[Math.floor(index / nameParts.length) % descriptorParts.length]
    const number = String(index + 1).padStart(3, '0')

    return {
      id: `${prefix}-${number}`,
      name: `${descriptor} ${namePart}`,
      description: `${descriptor} ${namePart.toLowerCase()} generated by DreamBuild's expanded catalog.`,
      category,
      files,
      tags: [category, descriptor.toLowerCase().replace(/\s+/g, '-'), namePart.toLowerCase().replace(/\s+/g, '-'), ...extraTags],
      popularity: 100 - (index % 35),
      difficulty: index % 5 === 0 ? 'Advanced' : index % 3 === 0 ? 'Intermediate' : 'Beginner',
      estimatedTime: index % 4 === 0 ? '30-60 minutes' : '10-30 minutes',
      source: 'DreamBuild Catalog',
      catalogIndex: index + 1
    }
  })
}

const GENERATED_STARTER_TEMPLATES = buildGeneratedTemplates(
  STARTER_TEMPLATE_COUNT,
  'starter',
  'starter-templates',
  [
    'Booking Site', 'Restaurant Website', 'Salon Website', 'Lawn Care Site', 'Fitness App', 'Church Website',
    'Real Estate Site', 'Travel Planner', 'Online Store', 'Portfolio', 'Medical Dashboard', 'School Portal',
    'Cleaning Service Site', 'Barber Booking App', 'Pet Care Site', 'Event Landing Page', 'Coach Website',
    'Course Portal', 'Finance Tracker', 'Community Hub', 'Auto Detail Site', 'Bakery Website', 'Daycare Site',
    'Repair Service Site', 'Nonprofit Website'
  ],
  ['Launch Ready', 'Premium', 'Mobile First', 'Animated', 'Admin Ready', 'Checkout Ready', 'Local Business', 'Modern', 'Conversion Focused', 'Content Rich', 'Client Portal', 'Service Pro'],
  ['index.html', 'styles.css', 'script.js', 'README.md'],
  ['starter', 'website', 'app']
)

const GENERATED_GAME_TEMPLATES = buildGeneratedTemplates(
  GAME_TEMPLATE_COUNT,
  'game',
  'games',
  [
    'Runner Game', 'Puzzle Game', 'Memory Match', 'Math Quest', 'Spelling Adventure', 'Racing Game',
    'Platformer', 'Space Shooter', 'Maze Game', 'Card Game', 'Trivia Game', 'Color Match',
    'Word Search', 'Tower Defense', 'Pet Care Game', 'Music Rhythm Game', 'Drawing Game', 'Cooking Game',
    'Treasure Hunt', 'Educational Quiz', 'Animal Rescue', 'Garden Game', 'Basketball Game', 'Soccer Game',
    'Dinosaur Adventure'
  ],
  ['Kids Safe', 'Arcade', 'Mobile Touch', 'Classroom', 'Family Friendly', 'Level Based', 'Canvas', 'Animated', 'Score Challenge', 'Beginner Friendly', 'Offline Ready', 'Colorful'],
  ['index.html', 'styles.css', 'script.js', 'assets.json'],
  ['game', 'canvas', 'mobile']
)

const GENERATED_FEATURE_TEMPLATES = buildGeneratedTemplates(
  FEATURE_TEMPLATE_COUNT,
  'feature',
  'code-features',
  [
    'Authentication Flow', 'Admin Dashboard', 'Stripe Checkout', 'Booking Calendar', 'Notification Center',
    'Image Uploader', 'Gallery Manager', 'Search Filters', 'Data Table', 'Chart Panel', 'User Profile',
    'Settings Page', 'Reviews Module', 'Chat Widget', 'Map Locator', 'Contact Form', 'Report Builder',
    'CRUD API', 'Database Schema', 'Email Template', 'Push Notifications', 'Inventory Manager',
    'Analytics Widget', 'Role Permissions', 'File Manager'
  ],
  ['Reusable', 'Production Ready', 'Accessible', 'Mobile Optimized', 'Secure', 'Animated', 'Admin Ready', 'Database Ready', 'API Ready', 'Form Validated', 'Realtime Ready', 'Themeable'],
  ['component.jsx', 'styles.css', 'schema.json', 'README.md'],
  ['feature', 'component', 'code']
)

const GENERATED_DESIGN_TEMPLATES = buildGeneratedTemplates(
  DESIGN_TEMPLATE_COUNT,
  'design',
  'designs',
  [
    'Editorial Layout', 'SaaS Dashboard', 'Luxury Landing Page', 'Playful Kids Theme', 'Bold Local Business',
    'Wellness Brand', 'Neon Game UI', 'Minimal Portfolio', 'Magazine Home Page', 'Premium Booking Flow',
    'Glassmorphism App', 'Retro Arcade Theme', 'Warm Restaurant Theme', 'Clean Medical UI', 'Travel Story Page',
    'Ecommerce Product Grid', 'Dark Developer Workspace', 'Bright Education Portal', 'Creator Studio',
    'Finance Command Center'
  ],
  [
    'Ocean Blue', 'Sunrise Orange', 'Forest Green', 'Midnight Slate', 'Candy Pop', 'Royal Gold',
    'Coral Peach', 'Arctic White', 'Cyber Lime', 'Berry Gradient', 'Sandstone', 'Lavender Mist',
    'High Contrast', 'Soft Pastel', 'Chrome Glass', 'Ink Editorial', 'Warm Neutral', 'Electric Cyan',
    'Rose Quartz', 'Deep Space'
  ],
  ['design-tokens.json', 'index.html', 'styles.css', 'preview-notes.md'],
  ['design', 'theme', 'layout']
)

const APP_ICON_NAMES = [
  'App Icon Set', 'Mobile Brand Icon', 'Launcher Icon', 'Store Icon Kit', 'Adaptive Icon',
  'Rounded App Mark', 'SaaS Product Icon', 'Game App Icon', 'Business App Icon', 'Wellness App Icon',
  'Creator App Icon', 'Utility App Icon', 'Finance App Icon', 'Travel App Icon', 'Education App Icon'
]

const FAVICON_NAMES = [
  'Favicon Pack', 'Browser Tab Icon', 'Website Brand Mark', 'PWA Favicon Kit', 'Minimal Favicon',
  'Gradient Favicon', 'Lettermark Favicon', 'Commerce Favicon', 'Portfolio Favicon', 'Dashboard Favicon',
  'Service Site Favicon', 'Blog Favicon', 'Game Site Favicon', 'Medical Favicon', 'Travel Favicon'
]

const IMAGE_NAMES = [
  'Hero Image', 'Product Graphic', 'Marketing Illustration', 'Feature Visual', 'Social Preview Image',
  'Landing Page Graphic', 'Dashboard Empty State', 'Game Background', 'Blog Cover', 'Ad Creative',
  'App Store Screenshot', 'Onboarding Art', 'Pricing Graphic', 'Testimonial Visual', 'Brand Pattern'
]

const ASSET_DESCRIPTORS = [
  'Aurora Blue', 'Solar Gold', 'Forest Neon', 'Midnight Pro', 'Candy Bright', 'Royal Violet',
  'Coral Studio', 'Arctic Clean', 'Cyber Lime', 'Berry Pop', 'Sandstone Calm', 'Lavender Soft',
  'High Contrast', 'Chrome Glass', 'Ink Modern', 'Electric Cyan', 'Rose Quartz', 'Deep Space',
  'Warm Neutral', 'Ocean Gradient'
]

const GENERATED_APP_ICON_TEMPLATES = buildGeneratedTemplates(
  ASSET_PREVIEW_TEMPLATE_COUNT,
  'appicon',
  'app-icons',
  APP_ICON_NAMES,
  ASSET_DESCRIPTORS,
  ['app-icon.svg', 'app-icon-preview.html', 'manifest-icons.json', 'README.md'],
  ['app-icon', 'logo', 'brand']
)

const GENERATED_FAVICON_TEMPLATES = buildGeneratedTemplates(
  ASSET_PREVIEW_TEMPLATE_COUNT,
  'favicon',
  'favicons',
  FAVICON_NAMES,
  ASSET_DESCRIPTORS,
  ['favicon.svg', 'favicon.ico.svg', 'browser-preview.html', 'README.md'],
  ['favicon', 'website-icon', 'brand']
)

const GENERATED_IMAGE_TEMPLATES = buildGeneratedTemplates(
  ASSET_PREVIEW_TEMPLATE_COUNT,
  'image',
  'image-generator',
  IMAGE_NAMES,
  ASSET_DESCRIPTORS,
  ['generated-image.svg', 'image-prompt.json', 'image-preview.html', 'README.md'],
  ['image', 'graphic', 'visual']
)

const EXPANDED_TEMPLATE_CATEGORIES = {
  'starter-templates': {
    name: 'Starter Templates',
    description: '300 ready-to-launch website and app starters',
    templates: GENERATED_STARTER_TEMPLATES
  },
  'games': {
    name: 'Game Templates',
    description: '300 playable game starters and mechanics',
    templates: GENERATED_GAME_TEMPLATES
  },
  'code-features': {
    name: 'Code & Feature Blocks',
    description: '300 reusable production feature modules',
    templates: GENERATED_FEATURE_TEMPLATES
  },
  'designs': {
    name: 'Design Directions',
    description: '5,000 visual design systems, themes, and layouts',
    templates: GENERATED_DESIGN_TEMPLATES
  },
  'app-icons': {
    name: 'App Icon Generator',
    description: 'Generates up to 40,000 app icon directions and export kits',
    templates: GENERATED_APP_ICON_TEMPLATES,
    totalCount: APP_ICON_GENERATOR_COUNT
  },
  'favicons': {
    name: 'Favicon Generator',
    description: 'Generates up to 40,000 favicon and browser icon kits',
    templates: GENERATED_FAVICON_TEMPLATES,
    totalCount: FAVICON_GENERATOR_COUNT
  },
  'image-generator': {
    name: 'Image Generator',
    description: 'Generates up to 40,000 image, graphic, and illustration prompts',
    templates: GENERATED_IMAGE_TEMPLATES,
    totalCount: IMAGE_GENERATOR_COUNT
  }
}
...
const LOCAL_AI_MODELS = {
  'auto': {
    ...
    languages: SUPPORTED_LANGUAGE_VALUES,
...
class LocalAIService {
  ...
  async getAllTemplates() {
    ...
    try {
      const githubTemplates = await Promise.race([
        githubTemplateService.getTrendingTemplates(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('GitHub template timeout')), 5000))
      ])
      ...
      console.warn('GitHub templates unavailable, using DreamBuild catalog:', error.message)
  ...
  async searchTemplates(query) {
    ...
    let transformedGitHubResults = []
    try {
      const githubResults = await Promise.race([
        githubTemplateService.searchTemplates(query),
        new Promise((_, reject) => setTimeout(() => reject(new Error('GitHub template timeout')), 5000))
      ])
      transformedGitHubResults = githubResults.map(repo => 
        githubTemplateService.transformRepositoryToTemplate(repo)
      )
    } catch (error) {
      console.warn('GitHub template search unavailable, using DreamBuild catalog:', error.message)
    }
  ...
  async getPopularTemplates() {
    ...
    let transformedGitHubPopular = []
    try {
      const githubPopular = await Promise.race([
        githubTemplateService.getPopularTemplates(5),
        new Promise((_, reject) => setTimeout(() => reject(new Error('GitHub template timeout')), 5000))
      ])
      transformedGitHubPopular = githubPopular.map(repo => 
        githubTemplateService.transformRepositoryToTemplate(repo)
      )
    } catch (error) {
      console.warn('GitHub popular templates unavailable, using DreamBuild catalog:', error.message)
    }
  ...
  async generateTemplateById(templateId, customizations = {}) {
    ...
    const template = localTemplates.find(t => t.id === templateId) || this.createVirtualAssetTemplate(templateId)
    ...
    const filesWithAssets = this.addBackgroundGeneratedAssets(files, template, customizations)
    console.log(`📁 Generated files: ${Object.keys(filesWithAssets).join(', ')}`)
    return filesWithAssets
  }

  getCatalogStats() {
    return {
      starterTemplates: STARTER_TEMPLATE_COUNT,
      gameTemplates: GAME_TEMPLATE_COUNT,
      codeFeatureTemplates: FEATURE_TEMPLATE_COUNT,
      designTemplates: DESIGN_TEMPLATE_COUNT,
      appIconGenerators: APP_ICON_GENERATOR_COUNT,
      faviconGenerators: FAVICON_GENERATOR_COUNT,
      imageGenerators: IMAGE_GENERATOR_COUNT,
      localCatalogTemplates: Object.values(TEMPLATE_CATEGORIES).reduce((total, category) => total + category.templates.length, 0)
    }
  }
...
<body>
  ...
  createVirtualAssetTemplate(templateId) {
    const match = /^(appicon|favicon|image)-(\d{1,5})$/.exec(templateId)
    if (!match) return null

    const [, prefix, rawNumber] = match
    const index = Number(rawNumber)
    const limits = {
      appicon: APP_ICON_GENERATOR_COUNT,
      favicon: FAVICON_GENERATOR_COUNT,
      image: IMAGE_GENERATOR_COUNT
    }

    if (!Number.isInteger(index) || index < 1 || index > limits[prefix]) return null

    const category = prefix === 'appicon' ? 'app-icons' : prefix === 'favicon' ? 'favicons' : 'image-generator'
    const names = prefix === 'appicon' ? APP_ICON_NAMES : prefix === 'favicon' ? FAVICON_NAMES : IMAGE_NAMES
    const files = prefix === 'appicon'
      ? ['app-icon.svg', 'app-icon-preview.html', 'manifest-icons.json', 'README.md']
      : prefix === 'favicon'
        ? ['favicon.svg', 'favicon.ico.svg', 'browser-preview.html', 'README.md']
        : ['generated-image.svg', 'image-prompt.json', 'image-preview.html', 'README.md']
    const namePart = names[(index - 1) % names.length]
    const descriptor = ASSET_DESCRIPTORS[Math.floor((index - 1) / names.length) % ASSET_DESCRIPTORS.length]

    return {
      id: `${prefix}-${String(index).padStart(3, '0')}`,
      name: `${descriptor} ${namePart}`,
      description: `${descriptor} ${namePart.toLowerCase()} generated by DreamBuild's asset generator.`,
      category,
      files,
      tags: [category, descriptor.toLowerCase().replace(/\s+/g, '-'), namePart.toLowerCase().replace(/\s+/g, '-'), 'asset-generator'],
      popularity: 80,
      difficulty: 'Beginner',
      estimatedTime: 'Instant',
      source: 'DreamBuild Asset Generator',
      catalogIndex: index
    }
  }

  getAssetPalette(index = 1) {
    const palettes = [
      { accent: '#3b82f6', secondary: '#06b6d4', dark: '#07111f' },
      { accent: '#f97316', secondary: '#facc15', dark: '#1c0f05' },
      { accent: '#10b981', secondary: '#84cc16', dark: '#04130d' },
      { accent: '#8b5cf6', secondary: '#ec4899', dark: '#13091f' },
      { accent: '#ef4444', secondary: '#f97316', dark: '#1f0707' },
      { accent: '#14b8a6', secondary: '#38bdf8', dark: '#04201d' }
    ]
    return palettes[(index - 1) % palettes.length]
  }

  createGeneratedCatalogTemplateFiles(template, customizations = {}) {
    const projectName = customizations.name || template.name || 'DreamBuild Project'
    const description = template.description || 'Generated with DreamBuild.'
    const palette = this.getAssetPalette(template.catalogIndex || 1)
    const accent = template.category === 'designs' ? '#ec4899' : template.category === 'games' ? '#f97316' : palette.accent
    const secondary = template.category === 'code-features' ? '#10b981' : palette.secondary

    if (['app-icons', 'favicons', 'image-generator'].includes(template.category)) {
      return this.createGeneratedAssetFiles(template, projectName, description)
    }

    if (template.category === 'code-features') {
      return {
        'component.jsx': `import React, { useState } from 'react'
import './styles.css'

export default function ${this.toComponentName(projectName)}() {
  const [active, setActive] = useState(true)

  return (
    <section className="feature-shell">
      <div>
        <p className="eyebrow">DreamBuild Feature</p>
        <h1>${projectName}</h1>
        <p>${description}</p>
      </div>
      <button onClick={() => setActive(!active)}>
        {active ? 'Feature Active' : 'Feature Paused'}
      </button>
    </section>
  )
}
`,
        'styles.css': `.feature-shell {
  display: grid;
  gap: 1rem;
  padding: 2rem;
  border-radius: 24px;
  color: #f8fafc;
  background: linear-gradient(135deg, #07111f, ${accent});
...
`,
        'schema.json': JSON.stringify({
          ...
          tags: template.tags,
          generatedBy: 'DreamBuild Catalog'
        }, null, 2),
        'README.md': `# ${projectName}

${description}

This generated feature starter includes a React component, CSS, schema metadata, and implementation notes.
`
      }
    }

    if (template.category === 'designs') {
      return {
        'design-tokens.json': JSON.stringify({
          name: projectName,
          colors: { background: '#07111f', surface: '#0f1d32', accent, secondary, text: '#f8fafc' },
...
<body>
  <main class="design-preview">
    <section class="hero-card">
      <p class="eyebrow">DreamBuild Design Direction</p>
      <h1>${projectName}</h1>
      <p>${description}</p>
      <div class="actions">
        <a href="#preview">Preview Layout</a>
        <a href="#tokens" class="secondary">View Tokens</a>
      </div>
    </section>
  </main>
</body>
</html>`,
        'styles.css': `@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@500;700;800&family=Space+Grotesk:wght@600;700&display=swap');
...
body {
  ...
  place-items: center;
  color: var(--text);
  background: radial-gradient(circle at 20% 20%, rgba(236,72,153,0.36), transparent 34rem), radial-gradient(circle at 80% 10%, rgba(139,92,246,0.32), transparent 30rem), var(--bg);
...
.hero-card {
  ...
  border-radius: 36px;
  background: linear-gradient(145deg, rgba(15,29,50,0.94), rgba(7,17,31,0.82));
...
`,
        'preview-notes.md': `# ${projectName}

${description}

Use this design direction as a visual foundation. It includes color tokens, typography choices, spacing, button treatments, and a responsive hero preview.
`
      }
    }
...
<body>
  <main class="game-shell">
    ...
    <canvas id="game" width="900" height="520" aria-label="${projectName} game canvas"></canvas>
    <p>${description}</p>
  </main>
  ...
  place-items: center;
  color: #f8fafc;
  background: radial-gradient(circle at top, ${accent}, #07111f 55%);
  font-family: Arial, sans-serif;
}

.game-shell { width: min(960px, calc(100vw - 2rem)); }
...
let player, stars, score, lives, keys

function reset() {
  player = { x: 80, y: 260, r: 24, speed: 5 }
  stars = Array.from({ length: 12 }, (_, i) => ({ x: 180 + i * 58, y: 70 + (i * 83) % 360, r: 12, taken: false }))
  score = 0
  lives = 3
  keys = {}
  updateHud()
...
function tick() {
  ...
  stars.forEach(star => {
    if (!star.taken && Math.hypot(player.x - star.x, player.y - star.y) < player.r + star.r) {
      star.taken = true
      score += 10
      updateHud()
    }
...
tick()
`,
        'assets.json': JSON.stringify({ sprites: [], sounds: [], notes: 'Add custom art and sounds here.' }, null, 2)
      }
    }
...
<body>
  ...
  <main>
    <section class="hero">
      <p class="eyebrow">DreamBuild Starter</p>
      <h1>${projectName}</h1>
      <p>${description}</p>
      <button id="cta">Start Project</button>
    </section>
    <section id="features" class="cards">
      <article><h2>Responsive</h2><p>Built to work on desktop, tablet, and mobile.</p></article>
      <article><h2>Polished</h2><p>Includes professional spacing, contrast, and clear actions.</p></article>
      <article><h2>Expandable</h2><p>Ready for forms, auth, dashboards, and database features.</p></article>
    </section>
  </main>
  <script src="script.js"></script>
</body>
...
.hero { padding: clamp(4rem, 10vw, 8rem) clamp(1rem, 4vw, 4rem); }
.eyebrow { color: ${accent}; font-weight: 900; letter-spacing: 0.16em; text-transform: uppercase; }
h1 { max-width: 12ch; margin: 0; font-size: clamp(3rem, 9vw, 7rem); line-height: 0.92; }
.hero p { max-width: 62ch; font-size: 1.15rem; line-height: 1.7; }
button { border: 0; border-radius: 999px; padding: 1rem 1.35rem; color: white; background: linear-gradient(135deg, ${accent}, ${secondary}); font-weight: 900; }
.cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1rem; padding: 0 clamp(1rem, 4vw, 4rem) 4rem; }
article { border: 1px solid rgba(15,23,42,0.1); border-radius: 24px; padding: 1.5rem; background: white; box-shadow: 0 18px 48px rgba(15,23,42,0.08); }
`,
      'script.js': `document.getElementById('cta')?.addEventListener('click', () => {
  alert('${projectName} is ready to customize in DreamBuild.')
})
`,
      'README.md': `# ${projectName}
...
This starter includes a responsive landing page, feature cards, styling, and a simple interaction hook.
`
    }
  }

  addBackgroundGeneratedAssets(files, template, customizations = {}) {
    if (!files || ['app-icons', 'favicons', 'image-generator'].includes(template.category)) {
      return files
    }

    const projectName = customizations.name || template.name || 'DreamBuild Project'
    const description = template.description || 'Generated with DreamBuild.'
    const seed = Math.max(1, Number(template.catalogIndex || 1))
    const appIconTemplate = this.createVirtualAssetTemplate(`appicon-${((seed - 1) % APP_ICON_GENERATOR_COUNT) + 1}`)
    const faviconTemplate = this.createVirtualAssetTemplate(`favicon-${((seed - 1) % FAVICON_GENERATOR_COUNT) + 1}`)
    const imageTemplate = this.createVirtualAssetTemplate(`image-${((seed - 1) % IMAGE_GENERATOR_COUNT) + 1}`)

    const appIconFiles = this.createGeneratedAssetFiles(appIconTemplate, projectName, description)
    const faviconFiles = this.createGeneratedAssetFiles(faviconTemplate, projectName, description)
    const imageFiles = this.createGeneratedAssetFiles(imageTemplate, projectName, description)

    return {
      ...files,
      'assets/app-icon.svg': appIconFiles['app-icon.svg'],
      'assets/favicon.svg': faviconFiles['favicon.svg'],
      'assets/hero-image.svg': imageFiles['generated-image.svg'],
      'assets/asset-generation.json': JSON.stringify({
        generatedBy: 'DreamBuild background asset generator',
        sourceTemplate: template.id,
        note: 'App icon, favicon, and hero image were generated automatically in the background for this project.',
        assets: {
  ...
  toComponentName(name) {
    const cleaned = String(name || 'DreamBuildComponent').replace(/[^a-zA-Z0-9]+/g, ' ')
    const pascal = cleaned.trim().split(/\s+/).map(part => part.charAt(0).toUpperCase() + part.slice(1)).join('')
    return /^[A-Za-z]/.test(pascal) ? pascal : `DreamBuild${pascal}`
  }

  createGeneratedAssetFiles(template, projectName, description) {
    const palette = this.getAssetPalette(template.catalogIndex || 1)
    const safeId = String(template.id || 'asset').replace(/[^a-z0-9-]/gi, '-')
    ...
    const iconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" role="img" aria-labelledby="title desc">
  <title id="title">${projectName}</title>
  ...
  <defs>
    ...
    <filter id="shadow" x="-20%" y="-20%" width="140%" height="140%">
      <feDropShadow dx="0" dy="34" stdDeviation="32" flood-color="#000" flood-opacity="0.28"/>
    </filter>
  </defs>
  <rect width="1024" height="1024" rx="236" fill="${palette.dark}"/>
  <circle cx="236" cy="214" r="190" fill="${palette.accent}" opacity="0.28"/>
  <circle cx="796" cy="804" r="250" fill="${palette.secondary}" opacity="0.22"/>
  <rect x="164" y="164" width="696" height="696" rx="188" fill="url(#bg)" filter="url(#shadow)"/>
  <path d="M314 572c92-8 166-82 174-174 8 92 82 166 174 174-92 8-166 82-174 174-8-92-82-166-174-174Z" fill="#fff" opacity="0.95"/>
  <text x="512" y="548" text-anchor="middle" dominant-baseline="middle" font-family="Arial, sans-serif" font-size="188" font-weight="900" fill="#fff">${initials}</text>
</svg>`

    if (template.category === 'app-icons') {
      return {
        'app-icon.svg': iconSvg,
        'app-icon-preview.html': this.createAssetPreviewHtml(projectName, description, 'app-icon.svg', 'App Icon Preview'),
        'manifest-icons.json': JSON.stringify({
          name: projectName,
          generatedBy: 'DreamBuild App Icon Generator',
          maxVariantsSupported: APP_ICON_GENERATOR_COUNT,
          sourceTemplate: template.id,
          recommendedExports: ['1024x1024', '512x512', '192x192', '180x180', '120x120', '87x87', '80x80', '76x76', '60x60', '40x40', '32x32', '16x16'],
          icons: [
...
DreamBuild generated this app icon kit from template ${template.id}. Export the SVG to platform sizes for iOS, Android, PWA, and app stores.
`
      }
    }

    if (template.category === 'favicons') {
      return {
        'favicon.svg': iconSvg,
        'favicon.ico.svg': iconSvg,
        'browser-preview.html': this.createAssetPreviewHtml(projectName, description, 'favicon.svg', 'Favicon Preview'),
        'README.md': `# ${projectName}

${description}

DreamBuild generated this favicon kit from template ${template.id}. Use favicon.svg for modern browsers and export 16x16, 32x32, and 48x48 ICO/PNG files when needed.
`
      }
    }

    const imageSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900" role="img" aria-labelledby="title desc">
  <title id="title">${projectName}</title>
  <desc id="desc">${description}</desc>
  <defs>
    <linearGradient id="sky" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="${palette.dark}"/>
      <stop offset="48%" stop-color="${palette.accent}"/>
      <stop offset="100%" stop-color="${palette.secondary}"/>
    </linearGradient>
  ...
  <circle cx="1200" cy="160" r="260" fill="url(#glow)"/>
  <path d="M0 680 C260 570 390 760 650 640 C900 526 1080 610 1600 470 L1600 900 L0 900 Z" fill="#07111f" opacity="0.86"/>
  <path d="M0 740 C290 650 510 820 790 700 C1040 590 1240 710 1600 610 L1600 900 L0 900 Z" fill="#fff" opacity="0.12"/>
  <rect x="110" y="110" width="650" height="280" rx="42" fill="#fff" opacity="0.12"/>
  <text x="150" y="210" font-family="Arial, sans-serif" font-size="72" font-weight="900" fill="#fff">${projectName}</text>
  <text x="154" y="298" font-family="Arial, sans-serif" font-size="34" fill="#e0f2fe">${description.slice(0, 96)}</text>
</svg>`

    return {
      'generated-image.svg': imageSvg,
      'image-prompt.json': JSON.stringify({
        name: projectName,
        generatedBy: 'DreamBuild Image Generator',
        maxVariantsSupported: IMAGE_GENERATOR_COUNT,
        sourceTemplate: template.id,
        prompt: `${projectName}. ${description}. Premium web graphic, professional composition, high contrast, responsive hero-ready image.`,
        formats: ['svg', 'png', 'webp', 'jpg']
      }, null, 2),
      'image-preview.html': this.createAssetPreviewHtml(projectName, description, 'generated-image.svg', 'Image Preview'),
      'README.md': `# ${projectName}

${description}

DreamBuild generated this image concept from template ${template.id}. Use the SVG directly or export to PNG/WebP/JPG for production.
`
    }
  }

  createAssetPreviewHtml(projectName, description, assetFile, label) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    body {
      ...
      place-items: center;
      color: #f8fafc;
      background: radial-gradient(circle at top, #1d4ed8, #07111f 58%);
      font-family: Arial, sans-serif;
    }
    main {
...
<body>
  <main>
    ...
    <h1>${projectName}</h1>
    <p>${description}</p>
    <img src="${assetFile}" alt="${projectName}">
  </main>
</body>
</html>`
  }
  ...
  createTemplateFiles(template, customizations = {}) {
    ...
    if (template.source === 'DreamBuild Catalog' || template.source === 'DreamBuild Asset Generator' || /^(starter|game|feature|design|appicon|favicon|image)-/.test(template.id)) {
      return this.createGeneratedCatalogTemplateFiles(template, customizations)
    }
