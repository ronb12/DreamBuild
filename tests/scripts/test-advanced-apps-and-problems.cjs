...
async function testAdvancedAppsAndProblems() {
  console.log('🚀 Testing AI Builder with 5 Advanced Complex Apps and 5 Difficult Coding Problems...');
  ...
  try {
    ...
    for (let i = 0; i < advancedApps.length; i++) {
      ...
      try {
        ...
        await page.evaluate(() => {
          const textarea = document.querySelector('textarea[placeholder*="Describe"], input[placeholder*="Describe"]');
          if (textarea) {
            textarea.value = '';
            textarea.focus();
          }
        ...
        // Type the prompt
        await page.type('textarea[placeholder*="Describe"], input[placeholder*="Describe"]', app.prompt);
        
        // Click generate button
        const generateButton = await page.$('[data-testid="generate-button"], button:has-text("Send"), button:has-text("Generate")');
        if (generateButton) {
    ...
    for (let i = 0; i < codingProblems.length; i++) {
      ...
      try {
        ...
        // Type the problem
        await page.type('textarea[placeholder*="Describe"], input[placeholder*="Describe"]', problem.problem);
    ...
    await page.screenshot({ 
      path: 'advanced-apps-and-problems-test.png',
      fullPage: true 
    });
    
    console.log('📸 Screenshot saved: advanced-apps-and-problems-test.png');
    ...
    console.error('❌ Advanced apps and problems test failed:', error);
...
testAdvancedAppsAndProblems().then(result => {
  if (result.error) {
    console.error('Advanced apps and problems test failed:', result.error);
  } else {
    console.log('\n🎉 Advanced apps and problems test completed!');
