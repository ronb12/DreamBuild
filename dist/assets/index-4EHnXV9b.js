...
`),commands:e,projectDir:s};return console.log("✅ Terminal commands executed successfully"),o}catch(o){return console.error("❌ Terminal command execution failed:",o),{success:!1,output:`Error: ${o.message}`,commands:e,error:o.message}}}}const nr=new au,lu=()=>{const{currentProject:r,switchFile:e,updateFile:s,saveProject:n,createNewProject:o,updateConfig:i}=_e(),[a,l]=p.useState(!1),[d,c]=p.useState(""),[u,m]=p.useState(!1),[x,y]=p.useState(!1),[g,f]=p.useState(!1),[h,S]=p.useState("firebase"),[b,j]=p.useState(!1),[w,v]=p.useState(""),[D,A]=p.useState({show:!1,x:0,y:0,filename:""}),M=p.useRef(null),B={"index.html":"🌐","style.css":"🎨","script.js":"⚡","components.jsx":"🧩","package.json":"📦","README.md":"📖","server.js":"🖥️","database.js":"🗄️","auth.js":"🔐","api.js":"🔌"},E=N=>B[N]||"📄",R=N=>{e(N)},k=(N,F)=>{N.preventDefault(),A({show:!0,x:N.clientX,y:N.clientY,filename:F})},_=N=>{const{filename:F}=D;switch(A({show:!1,x:0,y:0,filename:""}),N){case"download":q(F);break;case"delete":L(F);break;case"rename":$.info("Rename functionality coming soon!");break;case"copy":$.info("Copy functionality coming soon!");break}},T=()=>{A({show:!1,x:0,y:0,filename:""})};p.useEffect(()=>{const N=F=>{M.current&&!M.current.contains(F.target)&&T()};return D.show&&document.addEventListener("mousedown",N),()=>{document.removeEventListener("mousedown",N)}},[D.show]);const P=()=>{if(d.trim()){const N=d.includes(".")?d:`${d}.js`;s(N,""),e(N),c(""),l(!1),$.success(`Created ${N}`)}},L=N=>{if(Object.keys(r.files).length<=1){$.error("Cannot delete the last file");return}if(confirm(`Delete ${N}?`)){const F={...r.files};if(delete F[N],Object.keys(F).forEach(I=>{r.files[I]=F[I]}),r.activeFile===N){const I=Object.keys(F);e(I[0])}$.success(`Deleted ${N}`)}},q=N=>{const F=r.files[N]||"",I=new Blob([F],{type:"text/plain"}),K=URL.createObjectURL(I),re=document.createElement("a");re.href=K,re.download=N,document.body.appendChild(re),re.click(),document.body.removeChild(re),URL.revokeObjectURL(K),$.success(`Downloaded ${N}`)},G=()=>{const N={name:r.name,files:r.files,config:r.config,timestamp:new Date().toISOString()},F=new Blob([JSON.stringify(N,null,2)],{type:"application/json"}),I=URL.createObjectURL(F),K=document.createElement("a");K.href=I,K.download=`${r.name}.json`,document.body.appendChild(K),K.click(),document.body.removeChild(K),URL.revokeObjectURL(I),$.success("Project downloaded!")},ce=N=>{const F=N.target.files[0];if(F){const I=new FileReader;I.onload=K=>{s(F.name,K.target.result),e(F.name),$.success(`Uploaded ${F.name}`)},I.readAsText(F)}},ge=async()=>{if(!w.trim()){$.error("Please enter a project name");return}if(Object.keys(r.files).length===0){$.error("Please generate some code first");return}f(!0);try{const N=await U(r,w),F=[];if(b){$.info("Deploying to both Firebase and GitHub...");const[I,K]=await Promise.allSettled([nr.deployToFirebase(N,w),nr.deployToGitHub(N,w)]);if(I.status==="fulfilled"&&I.value.success&&F.push({platform:"Firebase",...I.value}),K.status==="fulfilled"&&K.value.success&&F.push({platform:"GitHub",...K.value}),F.length===2)$.success("Successfully deployed to both Firebase and GitHub!");else if(F.length===1)$.success(`Deployed to ${F[0].platform} (${F.length===1?"one":"both"} platform${F.length===1?"":"s"} failed)`);else throw new Error("Both deployments failed")}else{let I;h==="firebase"?I=await nr.deployToFirebase(N,w):h==="github"&&(I=await nr.deployToGitHub(N,w)),I.success&&(F.push({platform:h,...I}),$.success(`Successfully deployed to ${I.platform}!`))}F.forEach(I=>{I.url&&window.open(I.url,"_blank"),I.instructions&&(console.log(`${I.platform} deployment instructions:`,I.instructions),$.success(`Check console for ${I.platform} Pages setup instructions`))}),y(!1),v(""),j(!1)}catch(N){$.error(`Deployment failed: ${N.message}`)}finally{f(!1)}},U=async(N,F)=>{const I={...N};return I.files["index.html"]||(I.files["index.html"]=W(F)),I.files["package.json"]||(I.files["package.json"]=J(F,I.config)),I.files["README.md"]||(I.files["README.md"]=ne(F,I.config)),I.files["index.html"]=Pe(I.files["index.html"],F),I.files["manifest.json"]||(I.files["manifest.json"]=oe(F)),I},W=N=>`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
