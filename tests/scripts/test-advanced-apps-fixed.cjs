...
async function testAdvancedAppsFixed() {
  console.log('🚀 Testing AI Builder with 5 Advanced Complex Apps and 5 Difficult Coding Problems (Fixed)...');
  ...
  try {
    ...
    for (let i = 0; i < advancedApps.length; i++) {
      ...
      try {
        // Find the prompt input with multiple selectors
        let textarea = null;
        const selectors = [
        ...
        // Find and click generate button with multiple selectors
        ...
        if (!generateButton) {
          ...
          for (let j = 0; j < allButtons.length; j++) {
            ...
            if (text.includes('Send') || text.includes('Generate') || text.includes('Submit')) {
              generateButton = button;
              console.log(`✅ Found generate button at index ${j}: "${text}"`);
              break;
            }
          }
        }
    ...
    await page.screenshot({ 
      path: 'advanced-apps-fixed-test.png',
      fullPage: true 
    });
    
    console.log('📸 Screenshot saved: advanced-apps-fixed-test.png');
...
testAdvancedAppsFixed().then(result => {
  if (result.error) {
