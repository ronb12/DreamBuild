  ...
  async testMainTabs() {
    ...
    for (const tab of tabs) {
      try {
        ...
        if (elements && elements.asElement) {
          element = elements.asElement();
        }
        }

        if (element) {
          await element.click();
          await this.page.new Promise(resolve => setTimeout(resolve(1000);
  ...
  async testDashboard() {
    ...
    try {
      ...
      if (dashboardTab.length > 0) {
        ...
        await this.page.new Promise(resolve => setTimeout(resolve(2000);
      }
        ...
        try {
          ...
          for (let i = 0; i < Math.min(elements.length, 5); i++) {
            try {
              ...
              if (isVisible) {
                await element.click();
                await this.page.new Promise(resolve => setTimeout(resolve(500);
